...
import { Field, Input, Chip, PickerRow, PickerSheet, Avatar, BACK_ICON } from "../components";
...
export default function EditProfileScreen({
  ...
  const [kim, setKim] = useState({ favCats: user.favCats || [], positions: user.positions || [], levels: user.catLevels || {} });
  ...
  const kaydet = () => {
    ...
    else if (bolum !== "kimlik" && marketMine && marketMine.active) onDropListing();
    if (onSaveIdentity) onSaveIdentity(kim);
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: C.chalk }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 130 }} keyboardShouldPersistTaps="handled">
        ...
        {bolum !== "vitrin" && onSaveIdentity && (
          <Sec icon="🎯" title={t("BRANŞIN & MEVKİLERİN")}>
            {CATEGORIES.map((c) => {
              const acik = kim.favCats.includes(c.id);
              return (
                <View key={c.id} style={{ marginBottom: 10 }}>
                  <TouchableOpacity onPress={() => setKim((k) => {
                    const cats = acik ? k.favCats.filter((q) => q !== c.id) : [...k.favCats, c.id];
                    const izinli = cats.flatMap((x) => (POSITIONS[x] || []).map((p) => p.id));
                    return { ...k, favCats: cats, positions: k.positions.filter((p) => izinli.includes(p)) };
                  })}
                    style={{ flexDirection: "row", alignItems: "center", gap: 8, borderRadius: 12, borderWidth: 1.5, padding: 11, borderColor: acik ? C.turf : C.line, backgroundColor: acik ? C.pitchSoft : C.chalk }}>
                    <Text style={{ fontSize: 16 }}>{c.icon}</Text>
                    <Text style={{ fontSize: 14, fontWeight: "900", color: C.ink, flex: 1 }}>{t(c.name)}</Text>
                    <Text style={{ fontSize: 13, fontWeight: "900", color: acik ? C.turf : C.faint }}>{acik ? "✓" : "+"}</Text>
                  </TouchableOpacity>
                  {acik && (
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 5, marginTop: 8, marginLeft: 8 }}>
                      {LEVELS.map((l) => {
                        const sec = ((kim.levels || {})[c.id] || "Farketmez") === l;
                        return (
                          <TouchableOpacity key={l} onPress={() => setKim((k) => ({ ...k, levels: { ...k.levels, [c.id]: l } }))}
                            style={{ borderRadius: 999, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 5, borderColor: sec ? C.kit : C.line, backgroundColor: sec ? C.kit : C.chalk }}>
                            <Text style={{ fontSize: 11, fontWeight: "800", color: sec ? "#fff" : C.faint }}>{t(l)}</Text>
                          </TouchableOpacity>
                        );
                      })}
                    </View>
                  )}
                  {acik && (POSITIONS[c.id] || []).length > 0 && (
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 8, marginLeft: 8 }}>
                      {POSITIONS[c.id].map((p) => {
                        const sec = kim.positions.includes(p.id);
                        return (
                          <TouchableOpacity key={p.id} onPress={() => setKim((k) => ({ ...k, positions: sec ? k.positions.filter((q) => q !== p.id) : [...k.positions, p.id] }))}
                            style={{ borderRadius: 999, borderWidth: 1, paddingHorizontal: 11, paddingVertical: 7, borderColor: sec ? C.turf : C.line, backgroundColor: sec ? C.turf : C.chalk }}>
                            <Text style={{ fontSize: 12, fontWeight: "800", color: sec ? "#fff" : C.turfText }}>{p.icon} {t(p.label)}</Text>
                          </TouchableOpacity>
                        );
                      })}
                    </View>
