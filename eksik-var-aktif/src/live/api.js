...
const USER_COLS = "id, full_name, username, avatar_url, rating_avg, rating_count, reliability_pct, events_joined, events_organized, no_show_count, city_id, district_id, contact_mode, contact_scope, quiet_enabled, quiet_start, quiet_end, notif_basvuru, notif_mesaj, notif_hatirlatma, cities(name), districts(name)";
...
export function mapApplication(a, meId) {
  return {
    ...
    who: a.applicant_id === meId ? "me" : mapUser(a.applicant),
...
export async function listEvents(meId, cityId) {
  ...
  const ctx = { meId, joinedIds, attendance };
...
export async function createEvent(meId, f) {
  ...
  const { data, error } = await supabase.from("events").insert({
    ...
    recurrence: weekly ? "haftalik" : "yok", recurrence_until: until,
  }).select("id").single();
  if (error) throw error;
  return data.id;
}
...
export async function applyToEvent(meId, eventId, note) {
  const { data, error } = await supabase.from("applications").insert({ event_id: eventId, applicant_id: meId, message: note }).select("id, conversation_id").single();
  if (error) throw error;
  return data;
}
