...
export default function AuthScreen({ onSendCode, onVerify, busy = false, error = null, demoHint = null }) {
  ...
  const [f, setF] = useState({ name: "", username: "", phone: "", city: "Ankara", district: "", avatar: null, favCat: 1, positions: [] });
  ...
  const payload = () => ({ mode, phone: e164, name: f.name.trim(), username: f.username.trim(), city: f.city, district: f.district || null, avatar: f.avatar, favCat: f.favCat, positions: f.positions });
                ...
                <>
                  ...
                  <Field label={t("auth.where")}>
                    ...
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
                      {CATEGORIES.map((c) => (
                        <TouchableOpacity key={c.id} onPress={() => setF((x) => ({ ...x, favCat: c.id, positions: [] }))}
                          style={[st.cip, f.favCat === c.id && st.cipOn]}>
                          <Text style={{ fontSize: 12, fontWeight: "900", color: f.favCat === c.id ? "#fff" : C.turfText }}>{c.icon} {t(c.name)}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                    {(POSITIONS[f.favCat] || []).length > 0 && (
                      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                        {POSITIONS[f.favCat].map((p) => (
