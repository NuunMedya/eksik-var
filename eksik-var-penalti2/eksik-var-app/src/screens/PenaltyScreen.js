...
export default function PenaltyScreen({ onBack }) {
  ...
  const topY = useRef(new Animated.Value(120)).current;
  ...
  const sifirlaSahne = useCallback(() => {
    kaleciX.setValue(0); kaleciY.setValue(0); topX.setValue(0); topY.setValue(120); bant.setValue(0);
  ...
  const oynat = (hd, g, kaleci, benAtiyorum) => {
    ...
    const topHedefY = s.sonuc === "dis" ? Y.ust - 40 : s.sonuc === "direk" ? Y[hd.kat] - 6 : Y[hd.kat];
    Animated.parallel([
