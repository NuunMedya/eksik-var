...
const USER_COLS = "id, full_name, username, avatar_url, rating_avg, rating_count, reliability_pct, events_joined, events_organized, no_show_count, city_id, district_id, positions, team_name, is_verified, status, status_reason, suspended_until, contact_mode, contact_scope, quiet_enabled, quiet_start, quiet_end, notif_basvuru, notif_mesaj, notif_hatirlatma, cities(name), districts(name)";
...
export function mapUser(u) {
  ...
  return {
    ...
    positions: u.positions || [], teamName: u.team_name || null,
    contact: {
...
export async function listChats(meId, myApps) {
  ...
  if (convIds.length) {
    const { data: ps } = await supabase.from("polls").select("id, question, options, multiple, closed_at, created_by, poll_votes(user_id, option_id, users(full_name))").in("conversation_id", convIds);
    (ps || []).forEach((p) => {
      const votes = {};
      ...
      polls[p.id] = { id: p.id, question: p.question, options: p.options, multiple: !!p.multiple, closed: !!p.closed_at, createdBy: p.created_by === meId ? "me" : p.created_by, votes };
    });
  }
