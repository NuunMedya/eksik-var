...
export default function AuthScreen({ onLogin, busy = false, error = null }) {
  const [mode, setMode] = useState("login");
  const [f, setF] = useState({ name: "", username: "", email: "", phone: "", pass: "", city: "Ankara", district: "", avatar: null });
  const [picker, setPicker] = useState(null); // "il" | "ilce" | null
  const [agreed, setAgreed] = useState(false);
  const set = (k) => (v) => setF((s) => ({ ...s, [k]: v }));
  const canSubmit = (mode === "login" || agreed) && !busy;
    ...
    >
      <ScrollView contentContainerStyle={st.wrap} keyboardShouldPersistTaps="handled">
        ...
        <View style={st.card}>
            ...
            <>
              <View style={{ alignItems: "center", marginBottom: 14 }}>
                <AvatarPicker
                  ...
                  uri={f.avatar}
                  size={72}
                  onPress={() => chooseAvatar(f.avatar, (u) => set("avatar")(u))}
                />
                <Text style={{ fontSize: 11, color: C.faint, marginTop: 6 }}>
                  {f.avatar ? "Fotoğrafını değiştirmek için dokun" : "Fotoğraf ekle (isteğe bağlı)"}
                </Text>
              </View>
          ...
          <Field label="E-posta">
            <Input placeholder="ornek@eposta.com" keyboardType="email-address" autoCapitalize="none" autoCorrect={false} value={f.email} onChangeText={set("email")} />
          </Field>
          {mode === "register" && (
            <Field label="Telefon (isteğe bağlı, kimseyle paylaşılmaz)">
          ...
          {mode === "register" && (
            <TouchableOpacity onPress={() => setAgreed((a) => !a)} style={st.agreeRow} activeOpacity={0.8}>
              ...
              <Text style={st.agreeText}>
                ...
                <Text style={{ color: C.faint }}>(KVKK aydınlatma metni gizlilik sayfasındadır)</Text>
              </Text>
            </TouchableOpacity>
          )}

          <TouchableOpacity
            style={[st.cta, !canSubmit && { opacity: 0.45 }]}
            disabled={!canSubmit}
            onPress={() =>
              onLogin({
                mode,
          ...
          >
            <Text style={{ color: "#fff", fontWeight: "900", fontSize: 14 }}>
              {busy ? "Lütfen bekle…" : mode === "login" ? "Giriş yap" : "Kayıt ol ve başla"}
            </Text>
          </TouchableOpacity>
          {error ? (
            <Text style={{ textAlign: "center", color: C.kit, fontSize: 12, fontWeight: "700", marginTop: 10 }}>{error}</Text>
          ) : null}
...
const st = StyleSheet.create({
  ...
  agreeRow: { flexDirection: "row", alignItems: "flex-start", gap: 8, marginBottom: 12, marginTop: 2 },
