      ...
      <SafeAreaView edges={["left", "right", "bottom"]} style={{ flex: 1, backgroundColor: C.surface }}>
        <View style={{ flex: 1, backgroundColor: C.chalk }}>
          ...
          {view.name === "editProfile" && (
            <View style={StyleSheet.absoluteFill}>
              <EditProfileScreen onSaveIdentity={(f) => setUser((u) => ({ ...u, favCats: f.favCats, positions: f.positions }))} user={user} marketMine={myListing} onSaveListing={saveListing} onDropListing={dropListing} onBack={() => setView({ name: "root" })}
