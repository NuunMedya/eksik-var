...
export async function listPosts(meId, cityName) {
  ...
  let q = supabase.from("posts")
    .select("id, user_id, caption, image_url, video_url, attach_listing, created_at, users!posts_user_id_fkey(full_name, avatar_url, districts(name)), post_likes(user_id), post_comments(count)")
  ...
  return rows.map((r) => ({
    ...
    district: (r.users && r.users.districts && r.users.districts.name) || null,
...
export async function setDistrict(uid, cityName, districtName) {
  const cityId = PLAKA[cityName];
  if (!cityId || !districtName) return;
  const { data: d } = await supabase.from("districts").select("id").eq("city_id", cityId).eq("name", districtName).maybeSingle();
  if (d) { const { error } = await supabase.from("users").update({ district_id: d.id }).eq("id", uid); if (error) throw error; }
}
