...
t("penaltı 2.0 mantığı", () => {
  assert.equal(pn.BOLGELER.length, 6);
  const H = { sutun: "sol", kat: "alt" };
  assert.equal(pn.vurus(H, 0.7, { sutun: "sol", kat: "alt" }).sonuc, "kurtaris");   // aynı bölge
  assert.equal(pn.vurus(H, 0.7, { sutun: "sag", kat: "ust" }, () => 0.9).sonuc, "gol"); // ters köşe
  assert.equal(pn.vurus(H, 0.99, {}, () => 0.4).sonuc, "direk");                     // aşırı güç
  assert.equal(pn.vurus(H, 0.99, {}, () => 0.6).sonuc, "dis");
  assert.equal(pn.vurus(H, 0.2, { sutun: "sol", kat: "ust" }, () => 0.5).sonuc, "kurtaris"); // zayıf + aynı sütun
  assert.equal(pn.gucDegerlendir(0.6), "iyi");
});
...
t("smaç serisi mantığı", () => {
  assert.equal(vl.zamanla(300, 700, 150), "erken");
  assert.equal(vl.zamanla(700, 700, 150), "ideal");
  assert.equal(vl.zamanla(900, 700, 150), "gec");
  assert.equal(vl.smac("sol", "sag", "ideal"), "sayi");
  assert.equal(vl.smac("sol", "sol", "ideal"), "blok");
  assert.equal(vl.smac("sol", "sag", "erken"), "file");
  assert.ok(vl.volTur(10).sure < vl.volTur(0).sure);
});
...
t("duvar rallisi fiziği", () => {
  ...
  const sk = tn.sekme(0.1, 0.2, 120, 100, 40);
