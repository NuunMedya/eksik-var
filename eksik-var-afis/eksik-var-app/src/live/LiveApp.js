...
export default function LiveApp() {
  ...
  const saveListing = async (f) => { try { await api.upsertListing(meId, user, f); await refreshMarket(); setVitrinOniz(f); } catch (e) { fail(e); } };
  const [vitrinOniz, setVitrinOniz] = useState(null);
  const vitrinPaylas = async () => {
    const f = vitrinOniz; setVitrinOniz(null);
    try {
      await api.createPost(meId, me, { caption: (f.bio || "").trim() || t("🎯 Vitrindeyim — kadronda eksik varsa hazırım!"), attach: true });
      await refreshPosts();
    ...
    <View style={{ flex: 1, backgroundColor: C.turf }}>
      ...
      <SafeAreaView edges={["left", "right", "bottom"]} style={{ flex: 1, backgroundColor: C.surface }}>
        <View style={{ flex: 1, backgroundColor: C.chalk }}>
          ...
          {vitrinOniz && (
            <Modal visible transparent animationType="fade" onRequestClose={() => setVitrinOniz(null)}>
              <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.55)", justifyContent: "center", padding: 20 }}>
                <View style={{ backgroundColor: C.surface, borderRadius: 22, overflow: "hidden" }}>
                  <View style={{ backgroundColor: C.turf, paddingVertical: 9, alignItems: "center" }}>
                    <Text style={{ color: "#fff", fontWeight: "900", fontSize: 12, letterSpacing: 1 }}>{"\u{1F3AF} "}{t("VİTRİN GÖNDERİN — ÖNİZLEME")}</Text>
                  </View>
                  <View style={{ padding: 16 }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                      <Avatar name={me.name} uri={me.avatar} size={46} />
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontWeight: "900", fontSize: 16, color: C.ink }}>{me.name}</Text>
                        <Text style={{ fontSize: 12, color: C.faint }}>{user.city}{me.district ? " · " + me.district : ""}</Text>
                      </View>
                      <Text style={{ fontSize: 26 }}>{(CATEGORIES.find((k) => k.id === vitrinOniz.cat) || {}).icon || "\u26BD"}</Text>
                    </View>
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 12 }}>
                      {(vitrinOniz.positions || []).map((p2) => (
                        <View key={p2} style={{ backgroundColor: C.pitchSoft, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 5 }}>
                          <Text style={{ fontSize: 12, fontWeight: "800", color: C.turfText }}>{posIcon(p2)} {posLabel(p2)}</Text>
                        </View>
                      ))}
                      {me.catLevels && me.catLevels[vitrinOniz.cat] && (
                        <View style={{ backgroundColor: C.kitSoft, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 5 }}>
                          <Text style={{ fontSize: 12, fontWeight: "800", color: C.kit }}>{me.catLevels[vitrinOniz.cat]}</Text>
                        </View>
                      )}
                    </View>
                    {!!(vitrinOniz.bio || "").trim() && (
                      <Text style={{ fontSize: 13.5, color: C.ink, marginTop: 12, lineHeight: 19 }}>{vitrinOniz.bio.trim()}</Text>
                    )}
                    <Text style={{ fontSize: 11.5, color: C.faint, marginTop: 12 }}>{t("Beğendiysen paylaş — Keşfet'te 🎯 Oyuncu vitrini süzgecinde görünür, 💌 teklif alabilirsin.")}</Text>
                  </View>
                  <View style={{ flexDirection: "row", gap: 9, padding: 14, paddingTop: 0 }}>
                    <TouchableOpacity onPress={() => { setVitrinOniz(null); showToast(t("Vitrindesin! Takımlar seni görebilir 🏪")); }} style={{ flex: 1, backgroundColor: C.chalk, borderRadius: 14, paddingVertical: 12, alignItems: "center" }}>
                      <Text style={{ fontSize: 13.5, fontWeight: "800", color: C.faint }}>{t("Şimdilik kalsın")}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={vitrinPaylas} style={{ flex: 1.4, backgroundColor: C.kit, borderRadius: 14, paddingVertical: 12, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 6 }}>
