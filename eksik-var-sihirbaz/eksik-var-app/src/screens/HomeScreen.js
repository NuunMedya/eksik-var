...
export default function HomeScreen({ user, events, onOpen, onAttendance, onChangeCity, onNotifications, unreadCount = 0, blockedIds = [], onCreate = () => {}, onBringTeam = () => {}, loading = false, offline = false, sponsors = [], onSponsor = () => {} }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      <View style={st.kindRow}>
        {[["oyuncu", t("home.players")], ["rakip", t("home.opponents")]].map(([id, label]) => (
      ...
      <FlatList
        data={loading && list.length === 0 ? [] : injectSponsors(list, sponsors)}
        keyExtractor={(e) => e.id}
        ...
        ListEmptyComponent={loading ? null :
          ...
          <View style={st.empty}>
            ...
            <TouchableOpacity onPress={onCreate} style={[st.emptyBtn, { backgroundColor: C.kit }]}>
              ...
              <Text style={{ color: "#fff", fontWeight: "900", fontSize: 14 }}>{kind === "rakip" ? t("home.openOpponent") : t("home.openRequest")}</Text>
            </TouchableOpacity>
          </View>
          </View>
        }
