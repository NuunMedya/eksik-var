...
import { sonrakiSegment, karsiladiMi, sekme } from "../tenis";
...
export default function TennisScreen({ onBack }) {
  ...
  const segmentOynat = useCallback(() => {
    ...
    const cizgi = h - 44;                                   // raket çizgisi
      ...
      .start(({ finished }) => {
        ...
        if (s.tip === "alt") {
          ...
          if (karsiladiMi(topMerkez, raketRef.current, RAKET_YARIM + 6)) {
            const yeni = sekme(hiz.current.vx, hiz.current.vy, topMerkez, raketRef.current, RAKET_YARIM);
            hiz.current = yeni;
            setSkor((x) => x + 1);
          } else {
  ...
  const servis = () => {
    ...
    hiz.current = { vx: (Math.random() < 0.5 ? -1 : 1) * 0.14, vy: 0.24 };
    topXY.setValue(konum.current);
    canliRef.current = true;
    segmentOynat();
  };

  const basla = () => { setSkor(0); setCan(3); setFaz("oyun"); setTimeout(servis, 400); };
      ...
      {(faz === "oyun" || faz === "son") && (
        <View style={{ flex: 1 }}>
          ...
          <View style={st.alan} {...pan.panHandlers}
            ...
            {faz === "oyun" && skor === 0 && can === 3 && (
              <Text style={st.ipucu}>👆 {t("Parmağını sürükle")}</Text>
            )}
            {faz === "son" && (
...
const mkSt = () => StyleSheet.create({
  ...
  raket: { position: "absolute", bottom: 26, left: 0, width: RAKET_YARIM * 2, height: 12, borderRadius: 7, backgroundColor: "#FDE047" },
