...
export default function AttendanceScreen({ event, roster, onBack, onSave }) {
  ...
  const save = () => {
    ...
    Alert.alert(
      ...
      [{ text: t("Vazgeç"), style: "cancel" }, { text: t("Kaydet"), style: "destructive", onPress: () => onSave(marks) }]
    );
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      <View style={st.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: "#fff", fontWeight: "900", fontSize: 17 }}>{t("Yoklama")}</Text>
      ...
      <View style={st.summary}>
        <View style={st.sumItem}>
          ...
          <Text style={st.sumSub}>{t("katıldı")}</Text>
        </View>
        <View style={st.sumItem}>
          <Text style={[st.sumBig, { color: gelmedi ? C.kit : C.faint }]}>{gelmedi}</Text>
          <Text style={st.sumSub}>{t("gelmedi")}</Text>
        </View>
        <View style={[st.sumItem, { flex: 2 }]}>
          <Text style={{ fontSize: 11, color: C.faint, lineHeight: 15 }}>
            {t("Gelmeyenlere dokun. Kaydedince maç tamamlanır, kadro puanlama yapabilir.")}
          </Text>
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 140 }}>
              ...
              <View key={m.id} style={[st.row, i === roster.length - 1 && { borderBottomWidth: 0 }]}>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 4, marginTop: 1 }}>
                    ...
                    <Text style={{ fontSize: 11, color: C.faint }}>
                      {m.guest ? "uygulamada değil · yoklama kaydı tutulur" : `${relInfo(m).text} · ${m.via === "uygulama" ? "uygulamadan" : "ekipten"}`}{m.checkedIn ? " · 📍 sahadayım dedi" : ""}
                    </Text>
                  </View>
                </View>
                <View style={st.toggle}>
                  ...
                  >
                    ...
                    <Text style={[st.pillText, v === "katildi" && { color: "#fff" }]}>{t("Geldi")}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <Text style={[st.pillText, v === "gelmedi" && { color: "#fff" }]}>{t("Gelmedi")}</Text>
                  </TouchableOpacity>
                </View>
              </View>
            );
        ...
        <Text style={st.note}>
          {t("48 saat içinde yoklama alınmazsa sistem herkesi \"katıldı\" sayar ve maçı kendisi tamamlar. \"Gelmedi\" işaretlenen kişi itiraz edebilir.")}
        </Text>
      </ScrollView>
      ...
      <View style={st.bottomBar}>
        <TouchableOpacity onPress={save} style={st.cta}>
          ...
          <Text style={{ color: "#fff", fontWeight: "900", fontSize: 14 }}>
            {t("Yoklamayı kaydet ve maçı tamamla")}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
...
const mkSt = () => StyleSheet.create({
  ...
  sumSub: { fontSize: 11, color: C.faint },
