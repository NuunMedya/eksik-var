...
import { C } from "../theme";
import { CATEGORIES, GUNLER_UZUN, positionSlots, posLabel, posIcon, venueModeLabel, costModeLabel, teamLabel, VARMISIN_OPTIONS, PAYMENT_LABEL, paymentSummary } from "../data";
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 140 }}>
        <View style={{ flexDirection: "row", gap: 8 }}>
          {(rakip ? [
            ...
            ["people-outline", `${ev.format} · ${ev.level}`],
        ...
        {rakip && (
          <View style={[st.card, { borderLeftWidth: 4, borderLeftColor: C.kit }]}>
            ...
            <Text style={{ fontSize: 13, color: C.faint, marginTop: 2 }}>
              {ev.format} · {ev.level} · {venueModeLabel(ev.venueMode)}{ev.venue ? ` (${ev.venue})` : ""} · {costModeLabel(ev.costMode)}
            </Text>
            {ev.filled >= ev.needed && (
