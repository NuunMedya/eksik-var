...
export default function RateScreen({ event, people, onBack, onSubmit, mvpEnabled = true, myMvpVote = null }) {
  const [ratings, setRatings] = useState(() => Object.fromEntries(people.map((p) => [p.id, { stars: 4, comment: "" }]))); // varsayılan 4; değiştirmek istediğine dokun
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: C.chalk }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={st.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: "#fff", fontWeight: "900", fontSize: 17 }}>{t("Takım arkadaşlarını puanla")}</Text>
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 140 }} keyboardShouldPersistTaps="handled">
        ...
        <View style={{ flexDirection: "row", gap: 8, marginBottom: 10 }}>
          <TouchableOpacity onPress={() => setRatings((r) => Object.fromEntries(Object.entries(r).map(([k, v]) => [k, { ...v, stars: 5 }])))} style={st.quick}><Text style={st.quickText}>{t("Hepsine 5 ⭐")}</Text></TouchableOpacity>
          <TouchableOpacity onPress={() => setRatings((r) => Object.fromEntries(Object.entries(r).map(([k, v]) => [k, { ...v, stars: 4 }])))} style={st.quick}><Text style={st.quickText}>{t("Hepsine 4")}</Text></TouchableOpacity>
          <TouchableOpacity onPress={() => setRatings((r) => Object.fromEntries(Object.entries(r).map(([k, v]) => [k, { ...v, stars: 0 }])))} style={st.quick}><Text style={st.quickText}>{t("Temizle")}</Text></TouchableOpacity>
        </View>
        {mvpEnabled && (
          <View style={[st.card, { borderWidth: 1.5, borderColor: "#F5B301" }]}>
            <Text style={{ fontWeight: "900", fontSize: 14, color: C.ink }}>{t("🏆 Maçın oyuncusu (MVP)")}</Text>
          ...
          return (
            <View key={p.id} style={st.card}>
              ...
              {r.stars > 0 && (
                <TextInput
                  value={r.comment}
                  onChangeText={(v) => set(p.id, { comment: v })}
                  maxLength={200}
                  placeholder={t("Yorum (isteğe bağlı): dakiklik, oyun, saygı…")}
...
const mkSt = () => StyleSheet.create({
  ...
  quick: { flex: 1, alignItems: "center", backgroundColor: C.surface, borderWidth: 1, borderColor: C.line, borderRadius: 10, paddingVertical: 8 },
  quickText: { fontSize: 12, fontWeight: "800", color: C.turfText },
  mvpChip: { borderWidth: 1, borderColor: C.line, backgroundColor: C.surface, borderRadius: 999, paddingHorizontal: 12, paddingVertical: 7 },
