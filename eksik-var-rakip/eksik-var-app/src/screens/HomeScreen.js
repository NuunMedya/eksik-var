...
import { C } from "../theme";
import { CATEGORIES, GUNLER_UZUN, districtCounts, sortDistricts, needsSummary, matchesMyPositions, venueModeLabel, costModeLabel } from "../data";
    ...
    <TouchableOpacity onPress={() => onOpen(ev.id)} style={[st.card, { borderLeftWidth: 4, borderLeftColor: C.kit }]}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={st.sub}>{ev.format} · {ev.level} · {ev.date}</Text>
        </View>
        <EksikBadge ev={ev} />
      </View>
      <View style={[st.metaRow, { marginTop: 10 }]}>
        ...
        {ev.org && (<View style={st.meta}><Ionicons name="shield-checkmark" size={13} color={ev.org.rel < 85 ? C.kit : C.pitch} /><Text style={st.metaText}>%{ev.org.rel}</Text></View>)}
      </View>
    </TouchableOpacity>
  );
}
...
export default function HomeScreen({ user, events, onOpen, onAttendance, onChangeCity, onNotifications, unreadCount = 0, blockedIds = [] }) {
