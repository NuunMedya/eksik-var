        ...
        {onSaveIdentity && (
          <View style={{ marginBottom: 18 }}>
            ...
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
              {CATEGORIES.map((c) => (
                <TouchableOpacity key={c.id} onPress={() => { const cats = (user.favCats || []).includes(c.id) ? user.favCats.filter((q) => q !== c.id) : [...(user.favCats || []), c.id]; const izinli = cats.flatMap((k) => (POSITIONS[k] || []).map((p) => p.id)); onSaveIdentity({ favCats: cats, positions: (user.positions || []).filter((p) => izinli.includes(p)) }); }}
                  style={{ borderRadius: 999, borderWidth: 1, paddingHorizontal: 11, paddingVertical: 7, borderColor: (user.favCats || []).includes(c.id) ? C.turf : C.line, backgroundColor: (user.favCats || []).includes(c.id) ? C.turf : C.chalk }}>
                  <Text style={{ fontSize: 12, fontWeight: "900", color: (user.favCats || []).includes(c.id) ? "#fff" : C.turfText }}>{c.icon} {t(c.name)}</Text>
                </TouchableOpacity>
              ))}
            </View>
            {(user.favCats || []).some((k) => (POSITIONS[k] || []).length) && (
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                {(user.favCats || []).flatMap((k) => POSITIONS[k] || []).map((p) => (
                  <TouchableOpacity key={p.id} onPress={() => onSaveIdentity({ favCats: user.favCats || [], positions: (user.positions || []).includes(p.id) ? user.positions.filter((q) => q !== p.id) : [...(user.positions || []), p.id] })}
