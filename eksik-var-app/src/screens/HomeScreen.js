      ...
      <FlatList
        ...
        ListHeaderComponent={
          ...
          kind === "oyuncu" && upcomingMine.length > 0 ? (
            ...
            {kind === "oyuncu" && (
              <TouchableOpacity onPress={() => setKind("pazar")} activeOpacity={0.85}
                style={{ flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "#F5F3FF", borderWidth: 1.5, borderColor: "#7C3AED", borderRadius: 16, padding: 12, marginTop: 0, marginBottom: 12 }}>
                <View style={{ width: 38, height: 38, borderRadius: 12, backgroundColor: "#EDE9FE", alignItems: "center", justifyContent: "center" }}>
                  <Text style={{ fontSize: 18 }}>🔁</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 14, fontWeight: "900", color: "#7C3AED" }}>{t("Transfer Pazarı")}</Text>
                  <Text style={{ fontSize: 11, color: C.faint }}>{t("Takım arayan oyuncuları gör")}</Text>
                </View>
