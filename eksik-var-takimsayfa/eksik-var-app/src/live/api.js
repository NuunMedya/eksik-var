...
const USER_COLS = "id, full_name, username, bio, skill_level, avatar_url, rating_avg, rating_count, reliability_pct, events_joined, events_organized, no_show_count, city_id, district_id, positions, team_name, mvp_count, is_verified, status, status_reason, suspended_until, contact_mode, contact_scope, quiet_enabled, quiet_start, quiet_end, notif_basvuru, notif_mesaj, notif_hatirlatma, notif_yakin, cities(name), districts(name)";
...
export function mapUser(u) {
  ...
  return {
    ...
    positions: u.positions || [], teamName: u.team_name || null, mvpCount: u.mvp_count || 0,
...
export function mapChat(c, ctx) {
  ...
  return {
    ...
    muted: !!(me && me.is_muted), hasMore: raw.length >= 60,
...
export async function listEvents(meId, cityId) {
  ...
  (gs || []).forEach((g) => { (guests[g.event_id] = guests[g.event_id] || []).push({ id: g.guest_id, name: g.name, available: g.available, attendance: g.attendance, payment: g.payment_status, amount: Number(g.amount) }); });
...
/* ---------- sohbetler ---------- */
const CHAT_COLS = "id, type, event_id, series_id, name, created_at, pinned_message_id, pinned:messages!conversations_pinned_message_id_fkey(id, content, image_url), conversation_members(user_id, role, is_muted, last_read_message_id, users(" + USER_COLS + ")), messages!messages_conversation_id_fkey(id, sender_id, type, content, image_url, created_at, poll_id, reply_to_id, users!messages_sender_id_fkey(full_name), message_reactions(user_id, emoji))";
...
export async function myTeam(meId) {
  const { data } = await supabase.from("teams").select("id, name, emblem, category_id").eq("owner_id", meId).limit(1).maybeSingle();
  return data || null;
}
...
export async function myOffers(meId) {
  ...
  if (error) throw error;
  return (data || []).map((r) => ({
    id: r.id, kind: r.kind, status: r.status, message: r.message || "", createdAt: r.created_at,
    ...
    takim: r.teams ? `${r.teams.emblem} ${r.teams.name}` : null,
  }));
}
...
export async function listClub(cityName) {
  ...
  let q = supabase.from("club_listings")
    .select("team_id, bio, active, teams!inner(name, emblem, owner_id, city_id)")
    .eq("active", true);
  if (cityId) q = q.eq("teams.city_id", cityId);
  const { data, error } = await q.limit(40);
  if (error) throw error;
  return (data || []).map((r) => ({ teamId: r.team_id, bio: r.bio || "", name: r.teams.name, emblem: r.teams.emblem, ownerId: r.teams.owner_id }));
}
export async function upsertClubListing(teamId, bio, active = true) {
  const { error } = await supabase.from("club_listings").upsert({ team_id: teamId, bio: (bio || "").trim() || null, active, updated_at: new Date().toISOString() });
  if (error) throw error;
}
...
export async function listPosts(meId, cityName) {
  ...
  let q = supabase.from("posts")
    .select("id, user_id, caption, image_url, video_url, attach_listing, created_at, users!posts_user_id_fkey(full_name, avatar_url), post_likes(user_id)")
  ...
  if (ekliler.length) {
    const { data: ls } = await supabase.from("player_listings")
      .select("user_id, category_id, positions, users(rating_avg, reliability_pct)")
...
export async function addVenue(meId, cityName, categoryId, name, lat, lng) {
  ...
  if (error.code === "23505") {   // aynı ad zaten havuzda: onu seç
...
export async function vote(meId, pollId, optionId, selected) {
  const q = selected
    ? supabase.from("poll_votes").insert({ poll_id: pollId, user_id: meId, option_id: optionId })
...
export async function addGuest(meId, ev, name) {
  const row = ev.seriesId ? { owner_id: meId, series_id: ev.seriesId, name } : { owner_id: meId, event_id: ev.id, name };
