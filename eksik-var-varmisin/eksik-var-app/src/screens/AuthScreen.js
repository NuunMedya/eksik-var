    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: C.turf }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={st.wrap} keyboardShouldPersistTaps="handled">
        <View style={{ alignItems: "center", marginBottom: 20 }}>
          ...
          <Text style={{ color: C.mist, marginTop: 2, fontSize: 14, fontWeight: "600" }}>Kadron eksik kalmasın.</Text>
        </View>
            ...
            <>
              ...
              {mode === "register" && (
                <>
                  ...
                  <Field label="Ad Soyad"><Input placeholder="Emre Kaya" value={f.name} onChangeText={set("name")} /></Field>
                  <Field label="Kullanıcı adı"><Input placeholder="emre_k" autoCapitalize="none" value={f.username} onChangeText={set("username")} /></Field>
              ...
              <Field label="Cep telefonu">
                <Input placeholder="05xx xxx xx xx" keyboardType="phone-pad" textContentType="telephoneNumber" value={f.phone} onChangeText={set("phone")} />
                <Text style={{ fontSize: 11, color: C.faint, marginTop: 5 }}>
                  SMS ile 6 haneli kod göndereceğiz. Numaran hiçbir kullanıcıya gösterilmez.
                </Text>
              </Field>
              ...
              <TouchableOpacity style={[st.cta, busy && { opacity: 0.5 }]} disabled={busy} onPress={sendCode}>
                ...
                <Text style={st.ctaText}>{busy ? "Gönderiliyor…" : "SMS kodu gönder"}</Text>
              </TouchableOpacity>
            </>
          ) : (
            <>
              <Text style={{ fontSize: 17, fontWeight: "900", color: C.ink }}>Kodu gir</Text>
              <Text style={{ fontSize: 13, color: C.faint, marginTop: 4, lineHeight: 18 }}>
                {formatPhone(e164)} numarasına 6 haneli bir kod gönderdik.
              </Text>
              <Input
                placeholder="• • • • • •"
              ...
              <TouchableOpacity style={[st.cta, (busy || code.length < 6) && { opacity: 0.5 }]} disabled={busy || code.length < 6} onPress={verify}>
                ...
                <Text style={st.ctaText}>{busy ? "Doğrulanıyor…" : mode === "register" ? "Doğrula ve hesabı aç" : "Doğrula ve giriş yap"}</Text>
              </TouchableOpacity>
              <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 12 }}>
                <TouchableOpacity onPress={() => { setStep("form"); setLocalError(null); }}>
                  <Text style={{ fontSize: 12, fontWeight: "700", color: C.faint }}>Numarayı değiştir</Text>
                </TouchableOpacity>
                <TouchableOpacity disabled={cooldown > 0 || busy} onPress={sendCode}>
                  <Text style={{ fontSize: 12, fontWeight: "800", color: cooldown > 0 ? C.faint : C.pitch }}>
                    {cooldown > 0 ? `Yeniden gönder (${cooldown})` : "Kodu yeniden gönder"}
                  </Text>
                </TouchableOpacity>
              </View>
            </>
      ...
      <PickerSheet visible={picker === "il"} title="İl seç" items={ILLER} value={f.city}
        onSelect={(c) => { setF((s) => ({ ...s, city: c, district: "" })); setPicker(null); }} onClose={() => setPicker(null)} placeholder="İl ara…" />
      <PickerSheet visible={picker === "ilce"} title={`${f.city} ilçeleri`} items={ilceleri(f.city)} value={f.district}
        onSelect={(d) => { set("district")(d); setPicker(null); }} onClose={() => setPicker(null)} placeholder="İlçe ara…" />
