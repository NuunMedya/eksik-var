...
export async function listPosts(meId, cityName) {
  ...
  let q = supabase.from("posts")
    .select("id, user_id, caption, image_url, attach_listing, created_at, users(full_name, avatar_url), post_likes(user_id)")
  ...
  return rows.map((r) => ({
    ...
    caption: r.caption || "", image: r.image_url || null, createdAt: r.created_at,
...
export async function createPost(meId, user, { caption, imageUri, attach }) {
  let image_url = null;
  if (imageUri) {
  ...
  const { error } = await supabase.from("posts").insert({
    ...
    caption: (caption || "").trim() || null, image_url, attach_listing: !!attach,
  });
  if (error) throw error;
}
...
export async function listMarket(meId, cityName) {
  ...
  const { data, error } = await supabase.from("player_listings")
    .select("*, users(full_name, avatar_url, rating_avg, rating_count, reliability_pct), districts(name)")
    .eq("city_id", cityId).eq("active", true).neq("user_id", meId)
...
export async function sendImage(meId, conversationId, uri) {
  ...
  const form = new FormData(); form.append("file", { uri, name: "foto.jpg", type: "image/jpeg" });
  const { error: e1 } = await supabase.storage.from("chat").upload(path, form, { contentType: "image/jpeg" });
  if (e1) throw e1;
