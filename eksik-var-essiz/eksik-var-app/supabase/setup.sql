...
begin
  if exists (select 1 from information_schema.tables where table_schema = 'storage' and table_name = 'buckets') then
    ...
    execute $p$create policy p_storage_read on storage.objects for select using (bucket_id in ('avatars','chat','sponsors'))$p$;
  ...
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
    declare tbl text;
    begin
      foreach tbl in array array['messages','applications','events','conversation_members','notifications','poll_votes','polls','waitlist','message_reactions'] loop
        begin
