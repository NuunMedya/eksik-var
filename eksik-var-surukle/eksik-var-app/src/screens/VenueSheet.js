...
export default function VenueSheet({ cat = null, onCat = null, visible, onClose, cityName, categoryName, onList, onAdd, onPick, favoriler = [], onFav = null, initialQ = "" }) {
  ...
  const [merkezV, setMerkezV] = useState(null);
  ...
  useEffect(() => { if (visible && initialQ) setQ(initialQ); }, [visible]);
  useEffect(() => {
    if (!visible) return;
    let iptal = false;
    setYukleniyor(true);
  ...
  const noktayaEkle = (latitude, longitude) => {
    if (!onAdd) return;
  ...
  const sahaEkle = (e) => noktayaEkle(e.nativeEvent.coordinate.latitude, e.nativeEvent.coordinate.longitude);
    ...
    <Modal visible animationType="slide" onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: C.chalk }}>
        ...
        <MapView ref={mapRef} style={{ flex: 1 }} initialRegion={ANKARA}
          showsUserLocation={!!benim} onLongPress={(e) => { Keyboard.dismiss(); sahaEkle(e); }} onRegionChangeComplete={(r) => setMerkezV({ lat: r.latitude, lng: r.longitude })} onPress={(e) => { Keyboard.dismiss(); if (e.nativeEvent.action !== "marker-press") setSecili(null); }}>
          {pinli.map((v) => (
        ...
        {ekleModu && (
          <>
            <View pointerEvents="none" style={{ position: "absolute", top: "50%", left: "50%", marginLeft: -16, marginTop: -30, zIndex: 5 }}>
              <Text style={{ fontSize: 32 }}>{"\u{1F3AF}"}</Text>
            </View>
            <View style={st.modBant} pointerEvents="none">
              <Text style={{ fontSize: 12.5, fontWeight: "800", color: "#fff" }}>{t("Haritayı kaydır — iğneyi sahanın üstüne getir")}</Text>
            </View>
            <View style={st.modBar}>
              <TouchableOpacity onPress={() => setEkleModu(false)} style={[st.modBtn, { backgroundColor: C.chalk }]}>
                <Text style={{ fontSize: 13.5, fontWeight: "800", color: C.faint }}>{t("Vazgeç")}</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => { if (merkezV) { setEkleModu(false); noktayaEkle(merkezV.lat, merkezV.lng); } }} style={[st.modBtn, { backgroundColor: C.turf, flex: 1.4, flexDirection: "row", gap: 6 }]}>
                <Text style={{ fontSize: 15 }}>{"\u{1F4CC}"}</Text>
                <Text style={{ fontSize: 13.5, fontWeight: "900", color: "#fff" }}>{t("Bu noktaya ekle")}</Text>
              </TouchableOpacity>
            </View>
          </>
        )}
        {onAdd && !ekleModu && (
          <TouchableOpacity onPress={() => { setSecili(null); setEkleModu(true); }} style={st.ekleBtn}>
...
const mkSt = () => StyleSheet.create({
  ...
  modBant: { position: "absolute", top: 8, left: 40, right: 40, backgroundColor: "rgba(18,59,38,0.92)", borderRadius: 12, paddingHorizontal: 12, paddingVertical: 8, alignItems: "center", zIndex: 5 },
  modBar: { position: "absolute", left: 12, right: 12, bottom: 24, flexDirection: "row", gap: 9, zIndex: 6 },
  modBtn: { flex: 1, alignItems: "center", justifyContent: "center", borderRadius: 14, paddingVertical: 13, shadowColor: "#000", shadowOpacity: 0.16, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 4 },
  ekleBtn: { position: "absolute", right: 14, bottom: 186, width: 46, height: 46, borderRadius: 23, backgroundColor: C.kit, alignItems: "center", justifyContent: "center", shadowColor: "#000", shadowOpacity: 0.25, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 5 },
