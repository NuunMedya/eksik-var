...
export async function listEvents(meId, cityId) {
  ...
  const ctx = { meId, joinedIds, attendance, fill, waitCount, myWait, waitlists, availability, mvp, payments };
...
export async function confirmPayment(eventId, userId, status) { const { error } = await supabase.rpc("confirm_payment", { p_event: eventId, p_user: userId, p_status: status }); if (error) throw error; }
