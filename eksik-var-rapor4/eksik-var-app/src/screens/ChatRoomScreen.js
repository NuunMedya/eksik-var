...
export default function ChatRoomScreen({
  ...
  const longPress = (m) => {
    ...
    Alert.alert("Mesaj", (m.text || "Fotoğraf").slice(0, 60), [
      ...
      return (
        <View style={st.ibanCard}>
          ...
          <TouchableOpacity onPress={() => onCopy(ib.raw)} style={st.copyBtn}>
            ...
            <Text style={{ color: "#fff", fontWeight: "900", fontSize: 12 }}>IBAN'ı kopyala</Text>
          </TouchableOpacity>
        </View>
      );
    }
      ...
      <FlatList
        ...
        ListHeaderComponent={chat.hasMore && onLoadOlder ? (
          <TouchableOpacity onPress={() => onLoadOlder(chat.id)} disabled={loadingOlder} style={st.olderBtn}>
            <Text style={{ fontSize: 12, fontWeight: "800", color: C.turf }}>{loadingOlder ? "Yükleniyor…" : "Daha eski mesajları göster"}</Text>
          </TouchableOpacity>
        ) : null}
        ...
        <View style={st.inputBar}>
          ...
          {onSendImage && (
            <TouchableOpacity onPress={() => onSendImage(chat.id)} style={[st.pollBtn, { backgroundColor: C.chalk }]}>
              <Ionicons name="image-outline" size={20} color={C.turf} />
            </TouchableOpacity>
          )}
          <TextInput
            value={input}
...
const st = StyleSheet.create({
  ...
  ibanCard: { alignSelf: "center", backgroundColor: "#fff", borderRadius: 14, borderWidth: 1, borderColor: C.pitchSoft, padding: 12, marginVertical: 8, maxWidth: "88%" },
  ...
  imgWrap: { maxWidth: "75%", marginVertical: 4, backgroundColor: "#fff", borderRadius: 14, padding: 4, borderWidth: 1, borderColor: C.line },
  imgName: { fontSize: 11, fontWeight: "800", color: C.turf, marginLeft: 4, marginBottom: 2 },
