...
export default function EventDetailScreen({ onPinVenue = null, onUpdateDesc = null, onUpdateNeeds = null, onStopSeries = null, paymentDetails = null, onSaveIban = null,
  ...
  const showPay = ev.price > 0 && ev.status !== "iptal" && (ev.mine || !!ev.myPayment);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
        ...
        <View style={{ flexDirection: "row", backgroundColor: C.surface, borderRadius: 16, borderWidth: 1, borderColor: C.line, paddingVertical: 12, alignItems: "stretch" }}>
          {(rakip ? [
            ...
            <View key={i} style={{ flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 6, borderLeftWidth: i === 0 ? 0 : 1, borderLeftColor: C.line }}>
              <Text style={{ fontSize: 9.5, fontWeight: "900", letterSpacing: 1, color: C.faint }}>{[t("TARİH"), t("ÜCRET"), t("SEVİYE")][i]}</Text>
              <Text style={{ fontSize: 12.5, fontWeight: "900", color: i === 1 ? C.kit : C.ink, marginTop: 3, textAlign: "center" }} numberOfLines={2}>
                {text}
              </Text>
            </View>
          ))}
          ...
          <View style={st.card}>
            ...
            {guests.length === 0 && ev.mine && (
              <Text style={{ fontSize: 12, color: C.faint, marginTop: 6, lineHeight: 17 }}>
                Uygulamasız arkadaşını adıyla ekle — yoklamada, ödemede ve kurada yerini alır.
              </Text>
            )}
            {guests.map((g) => (
      ...
      {ev.venue && !(ev.venueLat > 35 && ev.venueLat < 43) && (
        ev.mine && onPinVenue ? (
          <TouchableOpacity onPress={() => onPinVenue(ev)} style={st.pinBant}>
            <Text style={{ fontSize: 16 }}>📍</Text>
            <Text style={{ flex: 1, fontSize: 12.5, fontWeight: "800", color: "#7A5800" }}>{t("Sahanın haritada yeri işaretsiz — iğnele ki gelenler yolu bulsun")}</Text>
            <Ionicons name="chevron-forward" size={15} color="#7A5800" />
          </TouchableOpacity>
        ) : (
          <View style={[st.pinBant, { backgroundColor: C.chalk }]}>
            <Text style={{ fontSize: 14 }}>📍</Text>
            <Text style={{ flex: 1, fontSize: 12, fontWeight: "700", color: C.faint }}>{t("Bu sahanın kesin konumu haritada işaretsiz — yol tarifi isimle arar")}</Text>
          </View>
        )
      )}
        {upcoming && (
              ...
              <View style={{ marginTop: 8 }}>
                {onSaveIban && (
                  <TouchableOpacity onPress={() => {
                    Alert.prompt && Alert.prompt(t("Alıcı adı"), t("Dekontta görünecek ad soyad"), [
                      { text: t("Vazgeç"), style: "cancel" },
                      { text: t("Devam"), onPress: (ad) => Alert.prompt(t("IBAN"), "TR__ ____ ____ ____ ____ ____ __", [
                        { text: t("Vazgeç"), style: "cancel" },
                        { text: t("Kaydet"), onPress: (ib) => { const tmz = (ib || "").replace(/\s/g, "").toUpperCase(); if (tmz.length >= 15) onSaveIban({ iban: tmz, holder: (ad || "").trim() || null }); } },
                      ], "plain-text", (paymentDetails && paymentDetails.iban) || ""),
                      },
                    ], "plain-text", (paymentDetails && paymentDetails.holder) || "");
                  }} style={{ flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: C.chalk, borderRadius: 12, padding: 10, marginBottom: 10 }}>
                    <Text style={{ fontSize: 15 }}>💳</Text>
                    {paymentDetails && paymentDetails.iban ? (
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 12.5, fontWeight: "800", color: C.ink }} numberOfLines={1}>{paymentDetails.holder || t("Alıcı adı yok")}</Text>
                        <Text style={{ fontSize: 11.5, color: C.faint }} numberOfLines={1}>{paymentDetails.iban}</Text>
                      </View>
                    ) : (
                      <Text style={{ flex: 1, fontSize: 12.5, fontWeight: "800", color: C.kit }}>{t("IBAN kayıtlı değil — ekle ki gruba gönderebilesin")}</Text>
                    )}
                ...
                {payList.length === 0 && (
                  <Text style={{ fontSize: 12, color: C.faint }}>{t("Katılımcı geldikçe ödeme listesi burada dolar.")}</Text>
                )}
                {payList.map((p) => (
        ...
        <View style={{ marginTop: 16 }}>
          {shareable && (
            <TouchableOpacity onPress={() => Alert.alert(t("Nasıl paylaşalım?"), t("Gruba giden davetin biçimini seç"), [
              ...
              { text: t("Vazgeç"), style: "cancel" },
            ])} style={[st.cta, st.waBtn, { marginTop: 2 }]}>
...
const mkSt = () => StyleSheet.create({
  ...
  pinBant: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: C.star, borderRadius: 14, paddingHorizontal: 12, paddingVertical: 11, marginBottom: 12 },
  bottomBar: {
