...
import { normalizePhone, formatPhone, CATEGORIES, POSITIONS } from "../data";
...
export default function AuthScreen({ onSendCode, onVerify, busy = false, error = null, demoHint = null }) {
  ...
  const [f, setF] = useState({ name: "", username: "", phone: "", city: "Ankara", district: "", avatar: null, favCats: [1], positions: [] });
  ...
  const payload = () => ({ mode, phone: e164, name: f.name.trim(), username: f.username.trim(), city: f.city, district: f.district || null, avatar: f.avatar, favCats: f.favCats, positions: f.positions });
              ...
              {mode === "register" && (
                <>
                  ...
                  <Field label={t("auth.where")}>
                    ...
                    <Text style={st.label}>{t("NE OYNUYORSUN?")}</Text>
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
                      {CATEGORIES.map((c) => (
                        <TouchableOpacity key={c.id} onPress={() => setF((x) => { const ic = x.favCats.includes(c.id); const cats = ic ? x.favCats.filter((q) => q !== c.id) : [...x.favCats, c.id]; const izinli = cats.flatMap((k) => (POSITIONS[k] || []).map((p) => p.id)); return { ...x, favCats: cats, positions: x.positions.filter((p) => izinli.includes(p)) }; })}
                          style={[st.cip, f.favCats.includes(c.id) && st.cipOn]}>
                          <Text style={{ fontSize: 12, fontWeight: "900", color: f.favCats.includes(c.id) ? "#fff" : C.turfText }}>{c.icon} {t(c.name)}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                    {f.favCats.some((k) => (POSITIONS[k] || []).length) && (
                      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                        {f.favCats.flatMap((k) => POSITIONS[k] || []).map((p) => (
                          <TouchableOpacity key={p.id} onPress={() => setF((x) => ({ ...x, positions: x.positions.includes(p.id) ? x.positions.filter((q) => q !== p.id) : [...x.positions, p.id] }))}
                            style={[st.cip, f.positions.includes(p.id) && st.cipOn]}>
                            <Text style={{ fontSize: 12, fontWeight: "800", color: f.positions.includes(p.id) ? "#fff" : C.turfText }}>{p.icon} {t(p.label)}</Text>
                          </TouchableOpacity>
                        ))}
                      </View>
                    )}
...
const mkSt = () => StyleSheet.create({
  cip: { borderRadius: 999, borderWidth: 1, borderColor: C.line, backgroundColor: C.chalk, paddingHorizontal: 11, paddingVertical: 7 },
  cipOn: { backgroundColor: C.turf, borderColor: C.turf },
