...
create table if not exists public.events (
  ...
  cost_mode             text check (cost_mode in ('yari_yariya','biz','siz')),
...
begin
  ...
  insert into public.users (id, full_name, username, city_id, district_id, is_verified)
  values (
...
grant select (id, full_name, username, bio, skill_level, avatar_url, positions, team_name,
...
grant update (full_name, username, bio, skill_level, avatar_url, positions, team_name,
...
grant insert (organizer_id, category_id, city_id, district_id, title, description, venue_name,
  ...
  offline_regulars) on public.events to authenticated;
grant update (category_id, city_id, district_id, title, description, venue_name, event_date,
  ...
  format, venue_mode, cost_mode, offline_regulars, recurrence_until) on public.events to authenticated;
...
begin
  if exists (select 1 from information_schema.tables where table_schema = 'storage' and table_name = 'buckets') then
    ...
    execute $p$create policy p_storage_read on storage.objects for select using (bucket_id in ('avatars','chat'))$p$;
    ...
    execute $p$create policy p_storage_write on storage.objects for insert to authenticated
      with check (bucket_id in ('avatars','chat') and (storage.foldername(name))[1] = auth.uid()::text)$p$;
  ...
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
    execute 'alter publication supabase_realtime add table public.messages, public.applications, public.events, public.conversation_members, public.notifications, public.poll_votes, public.polls, public.waitlist';
  end if;
