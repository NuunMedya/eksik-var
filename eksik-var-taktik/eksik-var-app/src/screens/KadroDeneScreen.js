...
import { View, Text, TouchableOpacity, TextInput, ScrollView, Share, ActivityIndicator, StyleSheet } from "react-native";
...
/* Kadro Dene 4.0 — üç perde: format → isimler → diziliş. */
const FORMATLAR = [5, 6, 7, 8, 9, 10, 11];
const TAKIM_CIFTLERI = [["Yelekliler", "Yeleksizler"], ["Şimşekler", "Kartallar"], ["Aslanlar", "Boğalar"], ["Kuzey", "Güney"]];
const FORMASYON = {
  5: ["1-2-1", "2-1-1", "1-1-2"],
...
export default function KadroDeneScreen({ onBack, onSearch = null, meName = null }) {
  ...
  const [adA, setAdA] = useState(TAKIM_CIFTLERI[0][0]);
  const [adB, setAdB] = useState(TAKIM_CIFTLERI[0][1]);
  ...
  const [aramaHedef, setAramaHedef] = useState(null); // "A" | "B" | null
  ...
  const formatSec = (n) => {
    ...
    const cift = TAKIM_CIFTLERI[Math.floor(Math.random() * TAKIM_CIFTLERI.length)];
    setAdA(cift[0]); setAdB(cift[1]);
  ...
  const kura = () => {
    const hepsi = [...boyut(listA), ...boyut(listB)].map((x) => x.trim()).filter(Boolean);
    for (let i = hepsi.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [hepsi[i], hepsi[j]] = [hepsi[j], hepsi[i]]; }
    const a = [], b = [];
    hepsi.forEach((ad, i) => (i % 2 === 0 ? a : b).push(ad));
    setListA(a); setListB(b);
  };
  ...
  const sonuctan = (u) => {
    const taraf = aramaHedef; if (!taraf) return;
    const set = taraf === "A" ? setListA : setListB;
  ...
  const paylas = () => {
    ...
    Share.share({ message: `\u26BD ${adA} vs ${adB} \u00B7 ${boy}v${boy}\n\n${satir(adA, formA, listA)}\n\n${satir(adB, formB, listB)}\n\n\u{1F517} ${APP_LINK}` }).catch(() => {});
  };
      ...
      <View style={{ position: "absolute", left: 0, right: 0, top: ust ? 0 : "50%", height: "50%" }}>
        {satirlar.map(({ ri, n }) => {
          ...
          return Array.from({ length: n }).map((_, j) => {
            ...
            return (
              <View key={ri + "-" + j} style={{ position: "absolute", left: ((j + 1) / (n + 1)) * 100 + "%", top: y + "%", marginLeft: -22, marginTop: -22, alignItems: "center", width: 44 }}>
                <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: bg, borderWidth: 2.5, borderColor: "rgba(0,0,0,0.35)", alignItems: "center", justifyContent: "center" }}>
                  <Text style={{ color: fg, fontWeight: "900", fontSize: 15 }}>{ad.trim().charAt(0).toUpperCase()}</Text>
                </View>
                <Text numberOfLines={1} style={{ color: "#fff", fontSize: 9.5, fontWeight: "800", marginTop: 2, maxWidth: 62, textShadowColor: "rgba(0,0,0,0.6)", textShadowRadius: 3 }}>{ad.split(" ")[0]}</Text>
              </View>
            );
          });
        })}
  ...
  if (adim === 2) return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
        <View style={{ flexDirection: "row", gap: 10 }}>
          {[["A", adA, setAdA, listA], ["B", adB, setAdB, listB]].map(([taraf, ad, setAd, list]) => (
            <View key={taraf} style={{ flex: 1 }}>
              <View style={{ backgroundColor: taraf === "A" ? C.turf : C.kit, borderRadius: 12, paddingHorizontal: 10, paddingVertical: 8, flexDirection: "row", alignItems: "center", gap: 6 }}>
                <TextInput value={ad} onChangeText={setAd} maxLength={16}
                  style={{ flex: 1, color: "#fff", fontWeight: "900", fontSize: 13.5, padding: 0 }} />
                {onSearch && (
        ...
        <TouchableOpacity onPress={kura} style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 12, backgroundColor: C.surface, borderWidth: 1.5, borderColor: C.line, borderRadius: 14, paddingVertical: 12 }}>
          <Text style={{ fontSize: 16 }}>{"\u2696\uFE0F"}</Text>
          <Text style={{ fontSize: 13.5, fontWeight: "900", color: C.ink }}>{t("Yazdıklarını kurayla dağıt")}</Text>
        </TouchableOpacity>
        {aramaHedef && (
          <View style={{ marginTop: 14, backgroundColor: C.surface, borderRadius: 16, borderWidth: 1, borderColor: C.line, padding: 12 }}>
            <Text style={{ fontSize: 12, fontWeight: "900", color: C.faint, marginBottom: 8 }}>{"\u{1F465} "}{t("Uygulamadan ekle")} \u2192 {aramaHedef === "A" ? adA : adB}</Text>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 30 }}>
        {[[adA, formA, setFormA, renkA, setRenkA], [adB, formB, setFormB, renkB, setRenkB]].map(([ad, form, setForm, renk, setRenk], k) => (
        ...
        <View style={{ borderRadius: 20, overflow: "hidden", marginTop: 6, backgroundColor: "#1B5E3B" }}>
          <View style={{ backgroundColor: "#123B26", paddingVertical: 10, alignItems: "center" }}>
            <Text style={{ color: "#fff", fontWeight: "900", fontSize: 15 }}>{adA} {t("vs")} {adB}</Text>
            <Text style={{ color: C.mist, fontSize: 11, fontWeight: "700" }}>{boy}v{boy} {"\u00B7"} Eksik Var</Text>
          </View>
        ...
        <TouchableOpacity onPress={paylas} style={[st.cta, { marginTop: 14, flexDirection: "row", justifyContent: "center", gap: 8 }]}>
          ...
          <Text style={{ color: "#fff", fontWeight: "900", fontSize: 14.5 }}>{t("Kadroyu paylaş")}</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
...
const mkSt = () => StyleSheet.create({
  ...
  altBar: { position: "absolute", left: 0, right: 0, bottom: 0, padding: 14, paddingBottom: 30, backgroundColor: C.surface, borderTopWidth: 1, borderTopColor: C.line },
