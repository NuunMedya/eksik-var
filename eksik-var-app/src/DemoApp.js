      ...
      <SafeAreaView edges={["left", "right", "bottom"]} style={{ flex: 1, backgroundColor: C.surface }}>
        <View style={{ flex: 1, backgroundColor: C.chalk }}>
          ...
          {view.name === "takim" && <TakimScreen meId="me"
            ...
            onUpdateNeeds={(id, needs) => setEvents((l) => l.map((e) => e.id === id ? { ...e, needs } : e))}
              onStopSeries={(id) => { setEvents((l) => l.map((e) => e.id === id ? { ...e, recurrence: "yok" } : e)); showToast(t("Seri durduruldu — bu hafta son \u23F9")); }}
              onUpdateDesc={(id, v) => setEvents((l) => l.map((e) => e.id === id ? { ...e, desc: v } : e))} onAddGuest={() => showToast(t("Misafir ekleme canlı modda"))} onRemoveGuest={() => {}} />}
