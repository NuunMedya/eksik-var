...
export default function LiveApp() {
  ...
  const teklifGonder = async (p) => { try {
    await api.createOffer({ kind: "oyuncu", toUser: p.userId, message: t("💌 Merhaba {p0}! Vitrinini beğendik, takımımızda oynamanı isteriz.", { p0: (p.name || "").split(" ")[0] }) });
    showToast(t("💌 Teklif gönderildi — Tekliflerim'den takip et"));
  } catch (e) { fail(e); } };
  const takimDaveti = async (kisi, tk) => { try {
    await api.createOffer({ kind: "takim", toUser: kisi.id, teamId: tk.id, message: t("🏆 {p0} seni kadrosuna davet ediyor!", { p0: tk.name }) });
  ...
  const hideTabs = ["chat", "create", "settings", "groupInfo", "attendance", "notifications", "rate", "blocked", "profile", "lineup", "editProfile", "penalti", "kadroDene", "basket", "voleybol", "tenis", "teklifler", "kulup"].includes(view.name);
  const ustEkran = ["hub", "penalti", "kadroDene", "basket", "voleybol", "tenis", "teklifler", "kulup"].includes(view.name) || !!activeEvent || !!call;
      ...
      <SafeAreaView edges={["left", "right", "bottom"]} style={{ flex: 1, backgroundColor: C.surface }}>
        <View style={{ flex: 1, backgroundColor: C.chalk }}>
          ...
          {view.name === "kulup" && <ClubBoardScreen rows={clubRows} meId={meId} myTeam={myTeamRow}
            ...
            onApply={async (r) => { try { await api.createOffer({ kind: "kulup", toUser: r.ownerId, teamId: r.teamId, message: t("🙋 Takımınıza katılmak istiyorum!") }); showToast(t("Başvurun iletildi 🙋")); } catch (e) { fail(e); } }}
            onPublish={async (bio) => { try { await api.upsertClubListing(myTeamRow.id, bio, true); await refreshClub(); showToast(t("İlan yayında 📣")); } catch (e) { fail(e); } }}
            onCloseListing={async () => { try { await api.upsertClubListing(myTeamRow.id, "", false); await refreshClub(); } catch (e) { fail(e); } }}
