  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      <View style={st.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: "#fff", fontWeight: "900", fontSize: 17 }}>{t("Kura")}</Text>
      ...
      <View style={st.bottomBar}>
        <TouchableOpacity onPress={() => reroll()} style={[st.btn, { backgroundColor: C.pitchSoft }]}>
          <Ionicons name="shuffle" size={18} color={C.turfText} />
          <Text style={[st.btnText, { color: C.turfText }]}>{t("Yeniden karıştır")}</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => onSend(text)} style={[st.btn, { backgroundColor: C.turf }]}>
          <Ionicons name="send" size={16} color="#fff" />
          <Text style={[st.btnText, { color: "#fff" }]}>{t("Gruba gönder")}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
const mkSt = () => StyleSheet.create({
  ...
  modeBtn: { borderRadius: 999, paddingHorizontal: 12, paddingVertical: 7, backgroundColor: C.surface, borderWidth: 1, borderColor: C.line },
  modeOn: { backgroundColor: C.turf, borderColor: C.turf },
  team: { flex: 1, backgroundColor: C.surface, borderRadius: 16, borderWidth: 1.5, overflow: "hidden" },
  ...
  bottomBar: { position: "absolute", bottom: 0, left: 0, right: 0, flexDirection: "row", gap: 8, backgroundColor: C.surface, borderTopWidth: 1, borderTopColor: C.line, padding: 14 },
