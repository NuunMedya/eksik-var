...
export default function KadroDeneScreen({ onBack, onSearch = null, meName = null }) {
  ...
  const yildizDondur = (name) => setHavuz((l) => l.map((x) => x.name === name ? { ...x, yildiz: (x.yildiz % 3) + 1 } : x));
  const ara = async () => {
  ...
  const kura = () => setTakimlar(makeTeams(havuz.map((p) => ({
    name: p.name, gk: p.gk, positions: p.positions || [],
    rating: p.id ? (p.rating || 0) : { 1: 2.5, 2: 3.5, 3: 4.5 }[p.yildiz || 2],
  })), "dengeli"));
  const paylas = () => {
    if (!takimlar) return;
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
        ...
        {sonuc.map((u) => (
          <TouchableOpacity key={String(u.id)} onPress={() => { ekle({ id: u.id, name: u.name, avatar: u.avatar, rating: u.rating || 0, positions: u.positions || [] }); setSonuc([]); setQ(""); }} style={st.sonucSatir}>
        ...
        <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
          <TextInput value={hizli} onChangeText={setHizli} placeholder={t("İsim yaz (uygulamasız arkadaş)")}
            placeholderTextColor={C.placeholder} style={[st.giris, { flex: 1 }]}
            onSubmitEditing={() => { if (hizli.trim()) { ekle({ name: hizli.trim(), yildiz: 2 }); setHizli(""); } }} returnKeyType="done" />
          <TouchableOpacity onPress={() => { if (hizli.trim()) { ekle({ name: hizli.trim(), yildiz: 2 }); setHizli(""); } }} style={[st.araBtn, { backgroundColor: C.kit }]}>
            <Ionicons name="add" size={20} color="#fff" />
          </TouchableOpacity>
        </View>
        {meName && !havuz.some((x) => x.name === meName) && (
          <TouchableOpacity onPress={() => ekle({ name: meName, yildiz: 2 })} style={{ marginTop: 8 }}>
            ...
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
              {havuz.map((p) => (
                <TouchableOpacity key={p.name} onPress={() => gkToggle(p.name)} onLongPress={() => cikar(p.name)}
                  ...
                  {!p.id && (
                    <TouchableOpacity onPress={() => yildizDondur(p.name)} hitSlop={{ top: 8, bottom: 8, left: 4, right: 4 }}
                      style={{ backgroundColor: C.star, borderRadius: 999, paddingHorizontal: 6, paddingVertical: 1, marginLeft: 5 }}>
                      <Text style={{ fontSize: 10, fontWeight: "900", color: "#7A5800" }}>{"⭐".repeat(p.yildiz || 2)}</Text>
                    </TouchableOpacity>
                  )}
                  {p.id && (p.rating || 0) > 0 && (
                    <Text style={{ fontSize: 10.5, fontWeight: "900", color: "#B8860B", marginLeft: 5 }}>★{p.rating}</Text>
                  )}
