...
drop policy if exists p_cm_self_update on public.conversation_members;
create policy p_cm_self_update on public.conversation_members for update to authenticated
  using (user_id = auth.uid()) with check (user_id = auth.uid());
grant update (is_muted, archived, hidden_at) on public.conversation_members to authenticated;

create or replace function public.rpc_create_group(p_name text, p_members uuid[])
returns bigint language plpgsql security definer set search_path = public as $$
declare v_id bigint; v_u uuid;
begin
  if auth.uid() is null then perform public.hata('giris_gerek'); end if;
  if p_name is null or char_length(trim(p_name)) < 2 or char_length(p_name) > 40 then perform public.hata('grup_adi'); end if;
  if coalesce(array_length(p_members, 1), 0) > 30 then perform public.hata('grup_cok_kalabalik'); end if;
  insert into public.conversations (type, name, created_by) values ('grup', trim(p_name), auth.uid()) returning id into v_id;
  insert into public.conversation_members (conversation_id, user_id, role) values (v_id, auth.uid(), 'yonetici');
  foreach v_u in array coalesce(p_members, '{}') loop
    if v_u is not null and v_u <> auth.uid() then
      insert into public.conversation_members (conversation_id, user_id) values (v_id, v_u) on conflict do nothing;
    end if;
  end loop;
  insert into public.messages (conversation_id, type, content) values (v_id, 'sistem', 'Grup kuruldu 👥');
  return v_id;
end $$;
revoke all on function public.rpc_create_group(text, uuid[]) from public;
...
alter table public.guests add column if not exists position text check (char_length(position) <= 20);
...
create view public.v_event_guests with (security_invoker = true) as
  select gr.event_id, g.id as guest_id, g.name, g.position, gr.available, gr.attendance,
...
-- takım grubu otomatiği: kurulunca grup, katılınca üyelik, ayrılınca çıkış
alter table public.conversations add column if not exists team_id uuid references public.teams(id) on delete set null;
create index if not exists idx_conv_team on public.conversations (team_id);

create or replace function public.trg_team_grubu_kur() returns trigger
language plpgsql security definer set search_path = public as $$
declare v_conv bigint;
begin
  insert into public.conversations (type, name, created_by, team_id)
  values ('grup', left('🛡 ' || new.name, 80), new.owner_id, new.id) returning id into v_conv;
  insert into public.conversation_members (conversation_id, user_id, role) values (v_conv, new.owner_id, 'yonetici');
  insert into public.messages (conversation_id, type, content) values (v_conv, 'sistem', 'Takım grubu kuruldu 🛡');
  return new;
end $$;
drop trigger if exists team_grubu_kur on public.teams;
create trigger team_grubu_kur after insert on public.teams
  for each row execute function public.trg_team_grubu_kur();

create or replace function public.trg_team_uyesi_gruba() returns trigger
language plpgsql security definer set search_path = public as $$
declare v_conv bigint; v_ad text;
begin
  select id into v_conv from public.conversations where team_id = new.team_id limit 1;
  if v_conv is null then return new; end if;
  insert into public.conversation_members (conversation_id, user_id) values (v_conv, new.user_id) on conflict do nothing;
  select full_name into v_ad from public.users where id = new.user_id;
  insert into public.messages (conversation_id, type, content) values (v_conv, 'sistem', coalesce(v_ad, 'Oyuncu') || ' takıma katıldı 👋');
  return new;
end $$;
drop trigger if exists team_uyesi_gruba on public.team_members;
create trigger team_uyesi_gruba after insert on public.team_members
  for each row execute function public.trg_team_uyesi_gruba();

create or replace function public.trg_team_uyesi_gruptan() returns trigger
language plpgsql security definer set search_path = public as $$
declare v_conv bigint; v_ad text;
begin
  select id into v_conv from public.conversations where team_id = old.team_id limit 1;
  if v_conv is null then return old; end if;
  delete from public.conversation_members where conversation_id = v_conv and user_id = old.user_id;
  select full_name into v_ad from public.users where id = old.user_id;
  insert into public.messages (conversation_id, type, content) values (v_conv, 'sistem', coalesce(v_ad, 'Oyuncu') || ' takımdan ayrıldı');
  return old;
end $$;
drop trigger if exists team_uyesi_gruptan on public.team_members;
create trigger team_uyesi_gruptan after delete on public.team_members
  for each row execute function public.trg_team_uyesi_gruptan();

-- mevcut takımlar için tek seferlik doldurma (grubu olmayanlara)
do $$
declare r record; v_conv bigint;
begin
  for r in select * from public.teams t where not exists (select 1 from public.conversations c where c.team_id = t.id) loop
    insert into public.conversations (type, name, created_by, team_id)
    values ('grup', left('🛡 ' || r.name, 80), r.owner_id, r.id) returning id into v_conv;
    insert into public.conversation_members (conversation_id, user_id, role) values (v_conv, r.owner_id, 'yonetici');
    insert into public.conversation_members (conversation_id, user_id)
      select v_conv, tm.user_id from public.team_members tm where tm.team_id = r.id
      on conflict do nothing;
    insert into public.messages (conversation_id, type, content) values (v_conv, 'sistem', 'Takım grubu kuruldu 🛡');
  end loop;
end $$;
...
grant update (caption, archived) on public.posts to authenticated;
...
create index if not exists ix_pc_post on public.post_comments (post_id, created_at);
alter table public.post_comments enable row level security;
drop policy if exists p_pc_read on public.post_comments;
create policy p_pc_read on public.post_comments for select to authenticated using (true);
drop policy if exists p_pc_ins on public.post_comments;
create policy p_pc_ins on public.post_comments for insert to authenticated with check (user_id = auth.uid());
drop policy if exists p_pc_del on public.post_comments;
create policy p_pc_del on public.post_comments for delete to authenticated
