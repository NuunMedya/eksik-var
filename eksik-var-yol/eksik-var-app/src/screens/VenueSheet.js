...
export default function VenueSheet({ visible, onClose, cityName, categoryName, onList, onAdd, onPick }) {
  ...
  const adresBul = async () => {
    ...
    try {
      const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=tr&q=${encodeURIComponent(q + ", " + cityName)}`, { headers: { "User-Agent": "EksikVar/1.0" } });
      const js = await r.json();
      if (js && js[0]) {
        const k = { latitude: Number(js[0].lat), longitude: Number(js[0].lon) };
        setPin(k); setAdresDurum(null);
        if (mapRef.current) mapRef.current.animateToRegion({ ...k, latitudeDelta: 0.01, longitudeDelta: 0.01 }, 600);
        ...
        <View style={st.sheet}>
          ...
          {mode === "harita" && (
            <View style={{ flex: 1 }}>
              ...
              {adresDurum === "yok" && <Text style={{ fontSize: 12, color: "#E24B4A", marginBottom: 6 }}>{t("Adres bulunamadı — biraz daha ayrıntı ekle")}</Text>}
...
const mkSt = () => StyleSheet.create({
  ...
  map: { height: Math.round(Dimensions.get("window").height * 0.52), borderRadius: 14, marginTop: 4 },
