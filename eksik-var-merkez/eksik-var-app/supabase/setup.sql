...
create table if not exists public.posts (
  ...
  created_at     timestamptz not null default now(),
  constraint posts_icerik check (caption is not null or image_url is not null)
);
...
begin
  ...
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
    declare tbl text;
    begin
      foreach tbl in array array['messages','applications','events','conversation_members','notifications','poll_votes','polls','waitlist','message_reactions','posts','post_likes'] loop
        begin
