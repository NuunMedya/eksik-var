...
import { C } from "../theme";
import { contactRules, DEFAULT_SETTINGS, buildShareText, buildInviteText, buildTeamInvite, extractIban, guestPlayers, mapsUrl, REPORT_REASONS, memberFromOrg, openPositions } from "../data";
...
const friendly = (e) => {
  ...
  if (m.includes("ZAMAN_DISI")) return "Sahadayım, maçtan 2 saat önce ile 4 saat sonra arasında denebilir";
...
export default function LiveApp() {
  ...
  useEffect(() => { if (me) saveCache(events, chats); }, [events, chats]); // eslint-disable-line
  ...
  useEffect(() => {
    ...
    (async () => {
      try { const raw = await AsyncStorage.getItem("ev_cache"); if (raw) { const c = JSON.parse(raw); if (c.events) setEvents(c.events); if (c.chats) setChats(c.chats); } } catch (e) { /* önbellek yok */ }
      try {
  ...
  useEffect(() => {
    ...
    const unsub = api.subscribe({
      ...
      onChange: async (what) => {
        if (what === "chats") { if (chatDebounce.current) clearTimeout(chatDebounce.current); chatDebounce.current = setTimeout(async () => { try { await refreshChats(); await refreshEvents(); } catch (e) { /* sessiz */ } }, 400); return; }
        try {
  ...
  const askPush = async () => {          // ilk kadroya giriş / ilk ilan sonrasında, gerekçesiyle
    ...
    Alert.alert("Maçı kaçırma", "Başvurun onaylanınca, kadro dolunca ve maça 2 saat kala haber verelim mi?", [
      { text: "Şimdi değil", style: "cancel", onPress: () => { pushAsked.current = false; } },
      { text: "Evet, bildir", onPress: async () => { AsyncStorage.setItem("ev_push_asked", "1").catch(() => {}); const token = await registerForPush(); if (token) api.savePushToken(meId, token); } },
    ]);
  };
  useEffect(() => {
    if (!meId) return;
  ...
  const confirmJoin = async (appId) => {
    try { await api.setApplication(appId, { applicant_approved: true }); const a = await refreshApps(); await Promise.all([refreshEvents(), refreshChats(undefined, a)]); showToast("Kadrodasın 🎉"); setTimeout(askPush, 1200); }
    catch (e) { fail(e); }
  };
  ...
  /* --- misafir oyuncular --- */
  const addGuest = async (eventId, name) => { const ev = events.find((e) => e.id === eventId); try { await api.addGuest(meId, ev, name); await refreshEvents(); showToast(`${name} eklendi`); } catch (e) { fail(e); } };
  ...
  const calendarFor = async (ev) => {
    try { await addToCalendar(ev); showToast("Takvime eklendi · maçtan 2 saat önce hatırlatır 📅"); }
    catch (e) { const m = String(e && e.message); showToast(m.includes("IZIN_YOK") ? "Takvim izni verilmedi" : "Takvime eklenemedi"); }
  };
  const directionsFor = (ev) => { const u = mapsUrl(ev); Linking.openURL(Platform.OS === "ios" ? u.ios : u.android).catch(() => showToast("Harita açılamadı")); };
  ...
  const checkIn = async (eventId) => { try { await api.checkIn(eventId); await refreshEvents(); showToast("Sahadayım dedin ✓"); } catch (e) { fail(e); } };
  const fixAttendance = async (eventId, userId) => { try { await api.fixAttendance(eventId, userId); setDisputes((d) => ({ ...d, [eventId]: (d[eventId] || []).filter((x) => x.userId !== userId) })); showToast("Yoklama düzeltildi ✓"); } catch (e) { fail(e); } };
  const canPinIn = (chat) => !!chat && chat.type === "grup" && (chat.members || []).some((m) => m.id === "me" && m.role === "organizator");
  useEffect(() => { if (activeEventRef.current && activeEventRef.current.mine && activeEventRef.current.status === "tamamlandi") api.eventDisputes(activeEventRef.current.id).then((d) => setDisputes((x) => ({ ...x, [activeEventRef.current.id]: d }))); }, [view]); // eslint-disable-line

  const copyToClipboard = async (text) => { const ok = await copyText(text); showToast(ok ? "IBAN kopyalandı 💳" : "Kopyalanamadı"); };
  const copyIbanFor = (eventId) => {
    ...
    if (!msg) { showToast("Organizatör henüz IBAN göndermedi"); return; }
  ...
  const startCall = () => showToast("Sesli arama yakında geliyor — şimdilik mesajlaşabilirsin");
  ...
  const callMember = () => showToast("Sesli arama yakında geliyor — şimdilik mesaj at");
  const callMemberOld = async (member) => {
  ...
  const confirmAllPayments = (eventId) => {
    ...
    Alert.alert("Herkes ödedi", `${pend.length} kişi 'ödendi' olarak işaretlenecek.`, [{ text: "Vazgeç", style: "cancel" }, { text: "İşaretle", onPress: async () => {
      try { for (const p of pend) await api.confirmPayment(eventId, p.id, "odendi"); await refreshEvents(); showToast("Tüm ödemeler işaretlendi ✓"); } catch (e) { fail(e); }
    } }]);
  };
  ...
  const hideTabs = ["chat", "create", "settings", "groupInfo", "attendance", "notifications", "rate", "blocked", "profile", "lineup", "editProfile"].includes(view.name) || !!activeEvent || !!call;
        ...
        <View style={{ flex: 1, backgroundColor: C.chalk }}>
          ...
          {tab === "chats" && view.name === "chat" && activeChat && (
            <ChatRoomScreen chat={activeChat} apps={apps} rules={rulesFor(activeChat)} onCall={() => startCall(activeChat)}
              ...
              onCreatePoll={createPoll} onVote={vote} onClosePoll={closePoll} onCopy={copyToClipboard}
          ...
          {activeEvent && (
            <View style={StyleSheet.absoluteFill}>
              <EventDetailScreen ev={activeEvent} apps={apps} myApp={myAppFor(activeEvent.id)} roster={activeEvent.ended ? rosterFor(activeEvent) : []}
                ...
                onCalendar={calendarFor} onDirections={directionsFor} onAddGuest={addGuest} onRemoveGuest={removeGuest} onToggleGuest={toggleGuest}
          ...
          {view.name === "create" && (
            <View style={StyleSheet.absoluteFill}>
              <CreateScreen user={user} preset={view.preset || null} initial={view.id ? events.find((e) => e.id === view.id) : null}
          ...
          {view.name === "editProfile" && (
            <View style={StyleSheet.absoluteFill}>
              <EditProfileScreen user={me} busy={profileBusy} error={profileError} onBack={() => setView({ name: "root" })}
                onSave={async (f) => { setProfileBusy(true); setProfileError(null); try { await api.updateProfileInfo(meId, f); await loadProfile(meId); setView({ name: "root" }); showToast("Profilin güncellendi"); } catch (e) { setProfileError(friendly(e)); } finally { setProfileBusy(false); } }} />
            </View>
          )}
          {view.name === "blocked" && (
            <View style={StyleSheet.absoluteFill}>
