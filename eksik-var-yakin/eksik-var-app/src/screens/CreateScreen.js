...
import { C } from "../theme";
import { CATEGORIES, nextDates, TIMES, GUNLER_UZUN, fmtEventDate, POSITIONS, FORMATS, VENUE_MODES, COST_MODES } from "../data";
...
export default function CreateScreen({ user, onBack, onCreate, initial = null }) {
  ...
  const [f, setF] = useState(() => initial ? {
    ...
    kind: initial.kind || "oyuncu", teamName: initial.teamName || user.teamName || "", format: initial.format || "7v7", offlineRegulars: String(initial.offlineRegulars || 0),
    ...
    kind: "oyuncu", teamName: user.teamName || "", format: "7v7", venueMode: "bizde", costMode: "yari_yariya", offlineRegulars: "0",
  });
  ...
  const submit = () => onCreate(rakip
    ? { ...f, title: f.title.trim() || `${f.format} rakip · ${GUNLER_UZUN[f.weekday]} ${f.time}`, needed: "1", capacity: String(parseInt(f.format, 10) || 7), needs: {}, recurrence: "yok" }
    : f);
  ...
  return (
    ...
    >
      <View style={st.header}>
        ...
        <View>
          <Text style={{ color: "#fff", fontWeight: "900", fontSize: 17 }}>{editing ? "Etkinliği düzenle" : rakip ? "Rakip ilanı aç" : "Eksik talebi aç"}</Text>
          ...
          <>
            ...
            <Field label="Format">
              <View style={{ flexDirection: "row", flexWrap: "wrap", rowGap: 8 }}>
                {FORMATS.map((fm) => <Chip key={fm} label={fm} active={f.format === fm} onPress={() => set("format")(fm)} />)}
              </View>
            </Field>
            <Field label="Saha">
              <View style={{ flexDirection: "row", flexWrap: "wrap", rowGap: 8 }}>
          ...
          <View style={{ flexDirection: "row", flexWrap: "wrap", rowGap: 8 }}>
            {CATEGORIES.map((c) => (
              <Chip
                ...
                onPress={() => setF((s) => ({ ...s, cat: c.id, needs: {} }))}
              />
            ))}
          </View>
        </Field>
      ...
      <View style={st.bottomBar}>
        ...
        >
          <Text style={{ color: "#fff", fontWeight: "900", fontSize: 14 }}>
            {editing ? "Değişiklikleri kaydet" : rakip ? "Rakip ilanını yayınla" : "Talebi yayınla"} · {fmtEventDate(f.dateISO, f.time)}
          </Text>
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
