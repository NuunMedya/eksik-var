...
export default function HomeScreen({ user, events, onOpen, onAttendance, onChangeCity, onNotifications, unreadCount = 0, blockedIds = [], onCreate = () => {}, onBringTeam = () => {}, loading = false, offline = false, sponsors = [], onSponsor = () => {}, market = [], onOpenPlayer = () => {}, onEditListing = () => {}, onOfferPlayer = null, initialKind = "oyuncu" }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      <View style={st.kindRow}>
        {[["oyuncu", t("home.players")], ["rakip", t("home.opponents")], ["pazar", t("home.market")]].map(([id, label]) => (
          <TouchableOpacity key={id} onPress={() => setKind(id)} style={[st.kindBtn, kind === id && st.kindOn]}>
            <Text style={{ fontSize: 13, fontWeight: "800", color: kind === id ? C.turfText : C.faint }}>{label}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <FlatList
        data={kind === "pazar" ? injectSponsors(marketFilter(market, { cat, district }), sponsors) : loading && list.length === 0 ? [] : injectSponsors(list, sponsors)}
        keyExtractor={(e) => e.id}
        ...
        ListHeaderComponent={
          kind === "oyuncu" && (pendingAttendance.length > 0 || upcomingMine.length > 0) ? (
            <View>
        ...
        ListEmptyComponent={loading ? null :
          ...
          <View style={st.empty}>
            ...
            <TouchableOpacity onPress={onBringTeam} style={st.emptyBtn}>
              ...
              <Text style={{ color: "#fff", fontWeight: "900", fontSize: 14 }}>{t("home.bringTeam")}</Text>
            </TouchableOpacity>
