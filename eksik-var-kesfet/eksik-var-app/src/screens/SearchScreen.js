...
export default function SearchScreen({ onSearchUsers, onSearchEvents, suggestions = [], recent = [], onAddRecent, onOpenUser, onOpenEvent, posts = [], onLikePost = () => {}, onDeletePost = null, onCompose = () => {} }) {
              ...
              {segment === "kisiler" && (
                <View style={{ marginTop: recent.length || suggestions.length ? 18 : 0 }}>
                  ...
                  {posts.length === 0 ? (
                    ...
                    <PostCard key={p.id} p={p} onLike={onLikePost} onDelete={onDeletePost}
