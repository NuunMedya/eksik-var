...
const USER_COLS = "id, full_name, username, bio, skill_level, avatar_url, rating_avg, rating_count, reliability_pct, events_joined, events_organized, no_show_count, city_id, district_id, positions, fav_cat, team_name, mvp_count, is_verified, status, status_reason, suspended_until, contact_mode, contact_scope, quiet_enabled, quiet_start, quiet_end, notif_basvuru, notif_mesaj, notif_hatirlatma, notif_yakin, cities(name), districts(name)";
...
export function mapUser(u) {
  ...
  return {
    ...
    positions: u.positions || [], favCat: u.fav_cat || null, teamName: u.team_name || null, mvpCount: u.mvp_count || 0,
...
export async function sendOtp({ mode, phone, name, username, city, district, favCat, positions }) {
  ...
  const { error } = await supabase.auth.signInWithOtp({
    ...
    options: {
      ...
      data: register ? { full_name: name, username, city_id: cityId ? String(cityId) : "", district_id: districtId ? String(districtId) : "", phone, fav_cat: favCat ? String(favCat) : "", positions: positions || [] } : undefined,
    },
  });
