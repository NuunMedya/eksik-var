...
export default function CallScreen({ call, onEnd, onAnswer = null }) {
  ...
  useEffect(() => {
    if (call.live) return; // canlı aramada durum dışarıdan gelir
    const t2 = setTimeout(() => setStatus("bağlandı"), 2500);
    return () => clearTimeout(t2);
  }, []);
  useEffect(() => {
    if (call.live && call.status) setStatus(call.status);
  }, [call.live, call.status]);
  useEffect(() => {
    if (!(call.live && status === "aranıyor")) return;
    const z = setTimeout(() => onEnd(0), 30000);   // 30 sn açılmazsa cevapsız
    ...
    <View style={st.wrap}>
      <View style={{ alignItems: "center", marginTop: 60 }}>
        ...
        <Text style={st.status}>
          {status === "gelen" ? t("Seni arıyor…") : status === "aranıyor" ? t("Aranıyor…") : fmt(seconds)}
        </Text>
        {call.live && status === "bağlandı" && (
          <Text style={{ color: C.star, fontSize: 12, fontWeight: "800", marginTop: 6 }}>{t("Konuşma sesi mağaza sürümüyle geliyor 📦 — şimdilik prova")}</Text>
        )}
      ...
      <View style={st.controls}>
        ...
        {status === "gelen" && onAnswer ? (
          <>
            <TouchableOpacity onPress={() => onEnd(0)} style={st.end}>
              <Ionicons name="call" size={30} color="#fff" style={{ transform: [{ rotate: "135deg" }] }} />
            </TouchableOpacity>
            <TouchableOpacity onPress={onAnswer} style={[st.end, { backgroundColor: "#22B45D" }]}>
