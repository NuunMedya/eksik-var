...
export default function ClubBoardScreen({ rows = [], meId, myTeam = null, myBio = "", appliedIds = [], onApply, onPublish, onCloseListing, onCreateTeam, onBack, rakipRows = [], onOpenRakip = () => {} }) {
  const [sekme, setSekme] = useState("oyuncu");
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      <View style={{ flexDirection: "row", gap: 8, paddingHorizontal: 14, paddingTop: 10 }}>
        {[["oyuncu", "🧍 " + t("Oyuncu arayanlar")], ["rakip", "🆚 " + t("Rakip arayanlar")]].map(([id, ad]) => (
          <TouchableOpacity key={id} onPress={() => setSekme(id)}
            style={{ flex: 1, alignItems: "center", paddingVertical: 9, borderRadius: 999, backgroundColor: sekme === id ? C.turf : C.surface, borderWidth: 1, borderColor: sekme === id ? C.turf : C.line }}>
            <Text style={{ fontSize: 12, fontWeight: "900", color: sekme === id ? "#fff" : C.faint }}>{ad}</Text>
          </TouchableOpacity>
        ))}
      </View>
      {sekme === "rakip" ? (
        <FlatList data={rakipRows} keyExtractor={(e) => String(e.id)}
          contentContainerStyle={{ padding: 14, paddingBottom: 40 }}
          renderItem={({ item }) => (
            <View style={st.kart}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                <View style={st.amblemKutu}><Text style={{ fontSize: 22 }}>🆚</Text></View>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 15, fontWeight: "900", color: C.ink }} numberOfLines={1}>{item.teamName || item.title}</Text>
                  <Text style={{ fontSize: 11, color: C.faint }}>{item.date} · {item.time}{item.district ? " · " + item.district : ""}</Text>
                </View>
              </View>
              <TouchableOpacity onPress={() => onOpenRakip(item)} style={[st.btn, { backgroundColor: C.pitch, marginTop: 10 }]}>
                <Text style={st.btnText}>🆚 {t("İlana bak")}</Text>
              </TouchableOpacity>
            </View>
          )}
          ListEmptyComponent={
            <View style={{ alignItems: "center", marginTop: 50, gap: 8 }}>
              ...
              <Text style={{ color: C.faint, fontSize: 13, textAlign: "center" }}>{t("Şu an rakip arayan takım yok — ilk ilanı sen aç!")}</Text>
            </View>
          } />
      ) : (<>
      ...
      <FlatList data={liste} keyExtractor={(r) => r.teamId}
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={st.kart}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                <View style={st.amblemKutu}>
                  {String(item.emblem || "").startsWith("http")
                    ? <Image source={{ uri: item.emblem }} style={{ width: 46, height: 46, borderRadius: 14 }} />
                    : <Text style={{ fontSize: 24 }}>{item.emblem}</Text>}
                </View>
                <View style={{ flex: 1 }}>
