...
export default function EventDetailScreen({
  ...
  onCalendar = () => {}, onDirections = () => {}, onAddGuest = () => {}, onRemoveGuest = () => {}, onToggleGuest = () => {},
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 140 }}>
        ...
        {!rakip && (ev.mine || guests.length > 0) && phase !== "iptal" && (
          <View style={st.card}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {ev.mine && <Text style={{ fontSize: 11, color: C.faint }}>bu hafta var / yok</Text>}
            </View>
            ...
            {guests.map((g) => (
              <View key={g.id} style={st.guestRow}>
                ...
                <View style={{ flex: 1, marginLeft: 8 }}>
                  <Text style={{ fontWeight: "800", fontSize: 13, color: g.available === false ? C.faint : C.ink }}>{g.name}</Text>
                ...
                {ev.mine && upcoming && (
                  <TouchableOpacity onPress={() => onToggleGuest(ev.id, g.id, g.available === false)} style={[st.payBtn, { backgroundColor: g.available === false ? C.chalk : C.pitchSoft }]}>
                    <Text style={{ fontSize: 11, fontWeight: "800", color: g.available === false ? C.faint : C.turf }}>{g.available === false ? "Yok" : "Var"}</Text>
                  </TouchableOpacity>
                )}
                {ev.mine && (
            ...
            {ev.mine && phase !== "tamamlandi" && (
              <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
                <TextInput value={guestName} onChangeText={setGuestName} placeholder="Ad Soyad" placeholderTextColor={C.placeholder} style={[st.scoreInput, { flex: 1, width: undefined, fontSize: 14, fontWeight: "600", textAlign: "left", paddingHorizontal: 12 }]} />
                <TouchableOpacity disabled={guestName.trim().length < 2} onPress={() => { onAddGuest(ev.id, guestName.trim()); setGuestName(""); }} style={[st.btn, { backgroundColor: C.turf, paddingHorizontal: 14, paddingVertical: 10 }, guestName.trim().length < 2 && { opacity: 0.4 }]}>
                  <Text style={[st.btnText, { color: "#fff" }]}>Ekle</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
        ...
        {ev.joined && !ev.mine && (phase === "bugun" || phase === "oynandi") && ev.status !== "tamamlandi" && (
          ev.checkedIn ? (
            <View style={[st.cta, { backgroundColor: C.pitchSoft, marginTop: 10, flexDirection: "row", justifyContent: "center", gap: 8 }]}>
              <Ionicons name="checkmark-circle" size={18} color={C.turf} /><Text style={[st.ctaText, { color: C.turf }]}>Sahadayım dedin ✓ · organizatör yoklamada görür</Text>
            </View>
          ) : (
            <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
              <TextInput value={codeInput} onChangeText={(t) => setCodeInput(t.replace(/\D/g, "").slice(0, 4))} keyboardType="number-pad" placeholder="Kod" placeholderTextColor={C.placeholder} style={[st.scoreInput, { width: 84 }]} maxLength={4} />
        ...
        {ev.mine && (phase === "bugun" || phase === "oynandi") && ev.status !== "tamamlandi" && (
          <TouchableOpacity onPress={() => onOpenCode(ev.id)} style={[st.card, { alignItems: "center", marginTop: 10, borderWidth: 1.5, borderColor: C.pitchSoft }]}>
            <Text style={st.cardTitle}>YOKLAMA KODU</Text>
            {checkinCode ? (
              <>
                <Text style={{ fontSize: 44, fontWeight: "900", color: C.turf, letterSpacing: 8, marginTop: 6 }}>{checkinCode}</Text>
              ...
              <Text style={{ fontSize: 13, fontWeight: "800", color: C.pitch, marginTop: 6 }}>Kodu göster · sahada söyle, herkes girsin</Text>
            )}
          </TouchableOpacity>
        )}
        {upcoming && (
          <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
            {ev.venue ? (
              <TouchableOpacity onPress={() => onDirections(ev)} style={[st.btn, st.btnGhost, { flex: 1, flexDirection: "row", gap: 6, paddingVertical: 10 }]}>
                <Ionicons name="navigate-outline" size={16} color={C.turf} /><Text style={[st.btnText, { color: C.turf }]}>{t("event.directions")}</Text>
              </TouchableOpacity>
            ) : null}
            {(ev.joined || ev.mine) && (
              <TouchableOpacity onPress={() => onCalendar(ev)} style={[st.btn, st.btnGhost, { flex: 1, flexDirection: "row", gap: 6, paddingVertical: 10 }]}>
                <Ionicons name="calendar-outline" size={16} color={C.turf} /><Text style={[st.btnText, { color: C.turf }]}>{t("event.calendar")}</Text>
              </TouchableOpacity>
            )}
          </View>
        )}
              ...
              <View style={{ marginTop: 8 }}>
                <View style={{ flexDirection: "row", gap: 8, marginBottom: 8 }}>
                  ...
                  <TouchableOpacity onPress={() => onRemindPayments(ev.id)} style={[st.btn, { flex: 1, backgroundColor: C.kitSoft, paddingVertical: 10, flexDirection: "row", gap: 6 }]}>
                    <Ionicons name="notifications-outline" size={16} color={C.kit} /><Text style={[st.btnText, { color: C.kit }]}>Hatırlat</Text>
                  </TouchableOpacity>
                </View>
                {payList.some((p) => p.status === "bekliyor" || p.status === "odedim") && (
                  <TouchableOpacity onPress={() => onConfirmAllPayments(ev.id)} style={[st.btn, { backgroundColor: C.pitchSoft, paddingVertical: 10, marginBottom: 8 }]}>
                    <Text style={[st.btnText, { color: C.turf }]}>Herkes ödedi · hepsini işaretle</Text>
                  </TouchableOpacity>
                )}
                {payList.map((p) => (
                ...
                {ev.myPayment.status === "bekliyor" && (
                  <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
                    <TouchableOpacity onPress={() => onCopyIban(ev.id)} style={[st.cta, st.btnGhost, { flex: 1, flexDirection: "row", justifyContent: "center", gap: 6 }]}>
                      <Ionicons name="copy-outline" size={15} color={C.turf} /><Text style={[st.ctaText, { color: C.turf }]}>IBAN'ı kopyala</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => onClaimPayment(ev.id)} style={[st.cta, { backgroundColor: C.turf, flex: 1 }]}>
                      <Text style={st.ctaText}>Ödedim</Text>
                    </TouchableOpacity>
                  </View>
                )}
            ...
            {disputes.map((d) => (
              <View key={d.userId} style={st.payRow}>
                ...
                <TouchableOpacity onPress={() => onFixAttendance(ev.id, d.userId)} style={[st.payBtn, { backgroundColor: C.pitch }]}>
                  <Text style={{ fontSize: 11, fontWeight: "800", color: "#fff" }}>Haklı · katıldı yap</Text>
                </TouchableOpacity>
              </View>
            ))}
            ...
            <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
              {ev.mine && (
                <TouchableOpacity onPress={() => setOpenStats((o) => !o)} style={[st.btn, st.btnGhost, { flex: 1, paddingVertical: 9 }]}>
                  <Text style={[st.btnText, { color: C.turf }]}>{openStats ? "Gol/asist girişini kapat" : "Gol / asist gir"}</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity onPress={() => onShareSummary(ev)} style={[st.btn, { flex: 1, backgroundColor: "#25D366", paddingVertical: 9, flexDirection: "row", gap: 6 }]}>
                <Ionicons name="logo-whatsapp" size={16} color="#fff" /><Text style={[st.btnText, { color: "#fff" }]}>Özeti gruba at</Text>
              </TouchableOpacity>
            </View>
            {ev.mine && openStats && (
              <View style={{ marginTop: 8 }}>
                {statRoster.map((m) => {
                  ...
                  return (
                    <View key={m.id} style={st.payRow}>
                      <Text style={{ flex: 1, fontWeight: "800", fontSize: 13, color: C.ink }} numberOfLines={1}>{m.name}{m.guest ? " ·" : ""}{m.guest ? <Text style={{ color: C.faint, fontWeight: "600" }}> misafir</Text> : null}</Text>
                      {[["⚽", "goals"], ["🎯", "assists"]].map(([ic, k]) => (
                        <View key={k} style={{ flexDirection: "row", alignItems: "center", marginLeft: 8 }}>
                          <Text style={{ fontSize: 12 }}>{ic}</Text>
                          <TouchableOpacity onPress={() => onSetStat(ev.id, m, { ...sv, [k]: Math.max(0, sv[k] - 1) })} style={st.stepBtn}><Ionicons name="remove" size={14} color={C.turf} /></TouchableOpacity>
                          <Text style={{ width: 18, textAlign: "center", fontWeight: "900", color: C.ink }}>{sv[k]}</Text>
                          <TouchableOpacity onPress={() => onSetStat(ev.id, m, { ...sv, [k]: Math.min(30, sv[k] + 1) })} style={st.stepBtn}><Ionicons name="add" size={14} color={C.turf} /></TouchableOpacity>
                        </View>
                      ))}
                    </View>
                  );
...
const mkSt = () => StyleSheet.create({
  ...
  card: { backgroundColor: C.surface, borderRadius: 16, padding: 14, marginTop: 12 },
