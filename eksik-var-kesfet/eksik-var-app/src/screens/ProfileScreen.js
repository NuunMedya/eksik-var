...
export default function ProfileScreen({ user, pendingRate, onRate, onLogout, onSettings, onInvite, onAvatar, settings = DEFAULT_SETTINGS, events = [], team = null, onTeam = () => {}, marketMine = null, onMarket = () => {}, onOpenEvent = () => {}, onPositionsChange = () => {}, onEdit = () => {}, onBringTeam = () => {} }) {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: C.chalk }} contentContainerStyle={{ paddingBottom: 120 }}>
      ...
      <TouchableOpacity onPress={onMarket} style={[st.teamRow, { borderWidth: 1.5, borderStyle: "dashed", borderColor: C.kit }]}>
        <View style={[st.emblem, { backgroundColor: C.kitSoft }]}>
          <Text style={{ fontSize: 20 }}>🏪</Text>
        </View>
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 11, fontWeight: "900", letterSpacing: 0.8, color: C.turfText }}>{t("TRANSFER PAZARI")}</Text>
          <Text style={{ fontWeight: "900", fontSize: 15, color: C.ink, marginTop: 1 }}>
            {marketMine && marketMine.active ? t("Vitrindesin") : t("Kendini vitrine çıkar")}
          </Text>
          <Text style={{ fontSize: 12, color: C.faint, marginTop: 1 }} numberOfLines={1}>
            {marketMine && marketMine.active ? (marketMine.bio || t("Takımlar seni görebiliyor")) : t("Takımlar seni bulsun; istatistiklerin kartında hazır")}
          </Text>
        </View>
          ...
          <View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 8, rowGap: 10 }}>
            {list.map((b) => (
              <TouchableOpacity
                ...
                style={{ width: "25%", alignItems: "center", opacity: b.earned ? 1 : 0.45 }}
              >
