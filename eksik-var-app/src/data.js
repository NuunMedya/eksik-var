...
export const SEED_CHATS = [
  ...
  },
  {
    ...
    unread: 0, lastTime: "14:06", pinned: { id: "pin1", text: "📍 Saha: Arena Spor Tesisleri, Keçiören · Kapı 2 · Otopark var" },
    ...
    msgs: [
      ...
      { id: "m2", from: "me", text: "Beyaz getirin, rakip koyu giyiyor", time: "14:02",
        reactions: { "👍": [{ id: "ozan", name: "Ozan Demir" }, { id: E2_TEAM[1].id, name: E2_TEAM[1].name }] } },
      { id: "m3", from: "ozan", name: "Ozan Demir", text: "Tamamdır, herkese yazıyorum 👕", time: "14:06",
        replyTo: { id: "m2", name: null, text: "Beyaz getirin, rakip koyu giyiyor" } },
    ],
  },
  {
...
// Tekrarla: aynı haftanın günü, bugünden sonraki ilk tarih
export const nextSameWeekday = (iso) => {
  const bugun = toDateISO(new Date());
  let d = iso;
  do { d = addDays(d, 7); } while (d <= bugun);
  return d;
};
export const cloneForRepeat = (ev) => ({ ...ev, id: undefined, dateISO: nextSameWeekday(ev.dateISO), recurrence: "yok" });
...
export function timeAgo(iso, now = Date.now()) {
  const fark = Math.max(0, (now - new Date(iso).getTime()) / 1000);
  if (fark < 60) return t("şimdi");
  if (fark < 3600) return Math.floor(fark / 60) + " " + t("dk önce");
  if (fark < 86400) return Math.floor(fark / 3600) + " " + t("sa önce");
  if (fark < 604800) return Math.floor(fark / 86400) + " " + t("gün önce");
  return new Date(iso).toLocaleDateString("tr-TR", { day: "numeric", month: "short" });
}
export const SEED_POSTS = [
  { id: "po1", userId: "zeynep", name: "Zeynep Arslan", createdAt: new Date(Date.now() - 35 * 60000).toISOString(),
    caption: "Bu sezon yeni takım arıyorum — pas kalitesine güvenim tam. Vitrinim aşağıda 👇",
    image: null, likes: ["ozan", "murat"], attach: true,
    listing: { cat: 3, positions: ["pasor"], rating: 4.9, rel: 98 } },
  { id: "po2", userId: "ozan", name: "Ozan Demir", createdAt: new Date(Date.now() - 5 * 3600000).toISOString(),
    caption: "Dün akşamki maçtan sonra üçlük çalışması 🏀 Hafta içi akşam açığım, gelen olursa yazsın.",
    image: null, likes: ["zeynep"], attach: false, listing: null },
  { id: "po3", userId: "murat", name: "Murat Şahin", createdAt: new Date(Date.now() - 26 * 3600000).toISOString(),
    caption: "İlk 10 maçım doldu, sıradaki hedef 50 🎯", image: null, likes: [], attach: false, listing: null },
];

/* ---------- transfer pazarı ---------- */
// Oyuncu arz ilanları: kişi kendini vitrine çıkarır; puan/istatistik profilden otomatik gelir.
export const SEED_MARKET = [
  { id: "pl1", userId: "murat", name: "Murat Şahin", rating: 4.2, count: 9, rel: 78,
    cat: 1, positions: ["forvet"], days: [4, 6], district: "Etimesgut",
    bio: "Hızlıyım, bitiriciyim. Perşembe-Cumartesi akşamları müsaidim." },
  { id: "pl2", userId: "zeynep", name: "Zeynep Arslan", rating: 4.9, count: 51, rel: 98,
    cat: 3, positions: ["pasor"], days: [2, 4], district: "Çankaya",
    bio: "Lig tecrübem var, düzenli takım arıyorum." },
  { id: "pl3", userId: "ozan", name: "Ozan Demir", rating: 4.5, count: 21, rel: 90,
    cat: 2, positions: ["guard"], days: [0, 3, 5], district: "Keçiören",
    bio: "Şutum iyidir, savunmada koşarım. Hafta içi akşam uygunum." },
];
export function marketFilter(list, { cat = 0, district = "Tümü", q = "" } = {}) {
  const n = String(q || "").toLocaleLowerCase("tr").trim();
  return (list || []).filter((p) =>
    (!cat || p.cat === cat) &&
    (district === "Tümü" || p.district === district) &&
    (!n || p.name.toLocaleLowerCase("tr").includes(n) || String(p.bio || "").toLocaleLowerCase("tr").includes(n)));
}
...
export const COST_MODES = [["yari_yariya", "Ücret yarı yarıya"], ["biz", "Biz karşılarız"], ["siz", "Siz karşılarsınız"], ["ucretsiz", "Ücretsiz"]];
...
/* ---------- sohbet+: tepkiler ve yanıtlar ---------- */
export const REACTION_EMOJIS = ["👍", "❤️", "😂", "😮", "⚽", "🔥"];
// Kullanıcı başına tek tepki: aynı emojiye ikinci dokunuş kaldırır, farklı emoji taşır
export function toggleReaction(reactions, emoji, user) {
  const out = {};
  let removedSame = false;
  for (const [e, list] of Object.entries(reactions || {})) {
    const filtered = list.filter((v) => v.id !== user.id);
    if (list.length !== filtered.length && e === emoji) removedSame = true;
    if (filtered.length) out[e] = filtered;
  }
  if (!removedSame) out[emoji] = [...(out[emoji] || []), { id: user.id, name: user.name }];
  return out;
}
export function myReaction(reactions, meId = "me") {
  for (const [e, list] of Object.entries(reactions || {})) if (list.some((v) => v.id === meId)) return e;
  return null;
}
// Görünüm sırası: çok olan önde, eşitse emoji listesindeki sıra
export const reactionList = (reactions) =>
  Object.entries(reactions || {}).filter(([, l]) => l && l.length)
    .sort((a, b) => b[1].length - a[1].length || REACTION_EMOJIS.indexOf(a[0]) - REACTION_EMOJIS.indexOf(b[0]));
// Yanıt kutusu için özet (name null → "Sen" olarak gösterilir)
export const replyPreview = (m) => ({
  id: m.id, dbId: m.dbId || null,
  name: m.from === "me" ? null : (m.name || null),
  text: String(m.text || (m.image ? "📷 Fotoğraf" : "")).slice(0, 90),
});
...
export const guestPlayers = (ev) => (ev.guests || []).filter((g) => g.available !== false).map((g) => ({ id: guestKey(g), name: g.name, rating: 4.0, count: 0, positions: g.gk ? ["kaleci"] : [], guest: true, role: "uye", via: "misafir" }));
export function mapsUrl(ev) {
  // Haritadan iğneyle seçilmişse tam koordinata git; yoksa ada göre ara
  if (ev.venueLat != null && ev.venueLng != null) {
    const ad = encodeURIComponent(ev.venue || "Saha");
    return {
      ios: `http://maps.apple.com/?ll=${ev.venueLat},${ev.venueLng}&q=${ad}`,
      android: `geo:${ev.venueLat},${ev.venueLng}?q=${ev.venueLat},${ev.venueLng}(${ad})`,
    };
  }
