...
const ar = await import("../src/arena.js");
t("arena bankası ve mantığı", () => {
  assert.ok(ar.SORULAR.length >= 40);
  ar.SORULAR.forEach((s) => { assert.equal(s.s.length, 4); assert.ok(s.d >= 0 && s.d <= 3); });
  const sec = ar.soruSec(10, () => 0.42);
  assert.equal(sec.length, 10);
  assert.equal(new Set(sec.map((x) => x.q)).size, 10);           // tekrarsız
  assert.equal(ar.dogruMu(ar.SORULAR[0], ar.SORULAR[0].d), true);
  assert.equal(ar.dogruMu(ar.SORULAR[0], (ar.SORULAR[0].d + 1) % 4), false);
});
console.log(`\n${n} test geçti`);
