      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 140 }}>
        <View style={{ flexDirection: "row", gap: 8 }}>
          {(rakip ? [
            ...
            <View key={i} style={st.infoBox}>
              <Ionicons name={icon} size={16} color={C.turfText} />
        ...
        {!rakip && (ev.mine || guests.length > 0) && phase !== "iptal" && (
          <View style={st.card}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {ev.mine && <Text style={{ fontSize: 11, color: C.faint }}>{t("bu hafta var / yok")}</Text>}
            </View>
            ...
            {ev.mine && phase !== "tamamlandi" && (
              <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
                <TextInput value={guestName} onChangeText={setGuestName} placeholder={t("Ad Soyad")} placeholderTextColor={C.placeholder} style={[st.scoreInput, { flex: 1, width: undefined, fontSize: 14, fontWeight: "600", textAlign: "left", paddingHorizontal: 12 }]} />
        ...
        <View style={st.card}>
          <TouchableOpacity onPress={() => setOpenDesc((o) => !o)} style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
            <Text style={st.cardTitle}>{t("AÇIKLAMA")}</Text>
