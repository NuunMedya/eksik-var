  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.turf }}>
      ...
      <Toast text={toast} />
          ...
          {/* alt sekme çubuğu */}
          {tab === "home" && view.name === "root" && (
            <TouchableOpacity activeOpacity={0.9}
              onPress={() => setView({ name: "create" })}
              onLongPress={() => {
                const son = events.find((e) => e.mine && e.kind !== "rakip");
                if (son) { showToast(t("🔁 Son ilanın kopyalandı — tarihi seç")); setView({ name: "create", repeat: true, initial: cloneForRepeat(son) }); }
                else setView({ name: "create" });
              }}
              style={{ position: "absolute", right: 18, bottom: 104, width: 58, height: 58, borderRadius: 29,
                backgroundColor: C.kit, alignItems: "center", justifyContent: "center",
