...
import { View, Text, TouchableOpacity, TextInput, Modal, ActivityIndicator, StyleSheet, ScrollView, Linking, Alert, Platform } from "react-native";
...
export default function VenueSheet({ cat = null, onCat = null, visible, onClose, cityName, categoryName, onList, onAdd, onPick }) {
  ...
  useEffect(() => {
    ...
    Promise.resolve(onList ? onList(q) : [])
      .then((r) => { if (!iptal) { setRows(r || []); setSecili(null); } })
      .catch(() => {})
      ...
      <View style={{ flex: 1, backgroundColor: C.chalk }}>
        ...
        <View style={st.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ color: C.mist, fontSize: 11 }}>{rows.length} {t("saha")} · {t("uzun bas, yeni saha ekle")}</Text>
          </View>
        ...
        <View style={st.aramaKutu}>
          ...
          <TextInput value={q} onChangeText={setQ} placeholder={t("Saha adı ara…")}
            placeholderTextColor={C.placeholder} style={{ flex: 1, fontSize: 14, color: C.ink, paddingVertical: 8 }} />
          {q.length > 0 && (
        ...
        <MapView ref={mapRef} style={{ flex: 1 }} initialRegion={ANKARA}
          showsUserLocation={!!benim} onLongPress={sahaEkle} onPress={() => setSecili(null)}>
          {rows.map((v) => (
            <Marker key={String(v.id)} coordinate={{ latitude: v.lat, longitude: v.lng }} onPress={() => setSecili(v)}>
              <View style={[st.pin, secili && secili.id === v.id && st.pinSecili]}>
                <Text style={{ fontSize: secili && secili.id === v.id ? 22 : 16 }}>{katIkon}</Text>
              </View>
            </Marker>
          ))}
        </MapView>
...
const mkSt = () => StyleSheet.create({
  ...
  pin: { backgroundColor: C.surface, borderRadius: 999, borderWidth: 2, borderColor: C.pitch, padding: 5 },
