    ...
    <ScrollView style={{ flex: 1, backgroundColor: C.chalk }} contentContainerStyle={{ paddingBottom: 120 }}>
      <View style={st.header}>
        ...
        <TouchableOpacity onPress={onEdit} style={{ flexDirection: "row", alignItems: "center", gap: 4, marginTop: 6, backgroundColor: "rgba(255,255,255,0.14)", borderRadius: 999, paddingHorizontal: 12, paddingVertical: 5 }}>
          ...
          <Text style={{ color: "#fff", fontSize: 12, fontWeight: "800" }}>Profili düzenle</Text>
        </TouchableOpacity>
        ...
        {user.mvpCount > 0 && (
          <TouchableOpacity onPress={() => Alert.alert("MVP", "Tamamlanan maçlarda takım arkadaşların 'maçın oyuncusu' oyu verir; oylar 48 saat sonra sayılır, en çok oyu alan MVP olur.")} style={{ flexDirection: "row", alignItems: "center", gap: 4, marginTop: 6, backgroundColor: "rgba(245,179,1,0.18)", borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4 }}>
            <Text style={{ fontSize: 12 }}>🏆</Text>
            <Text style={{ color: "#fff", fontSize: 11, fontWeight: "800" }}>MVP ×{user.mvpCount}</Text>
          </TouchableOpacity>
        )}
        {user.verified && (
          <TouchableOpacity onPress={() => Alert.alert("Telefonu doğrulanmış", "Bu hesap SMS koduyla doğrulanmış gerçek bir numaraya bağlı; kapatılan hesaplar aynı numarayla geri dönemez.")} style={{ flexDirection: "row", alignItems: "center", gap: 4, marginTop: 6, backgroundColor: "rgba(255,255,255,0.12)", borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4 }}>
            <Ionicons name="shield-checkmark" size={13} color={C.star} />
            <Text style={{ color: "#fff", fontSize: 11, fontWeight: "800" }}>Telefonu doğrulanmış</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={st.card}>
        <Text style={st.cardTitle}>MEVKİLERİM</Text>
        ...
        {Object.entries(POSITIONS).map(([catId, list]) => (
          <View key={catId} style={{ marginTop: 6 }}>
            ...
            <View style={{ flexDirection: "row", flexWrap: "wrap", rowGap: 8 }}>
              {list.map((pos) => <Chip key={pos.id} label={`${pos.icon} ${pos.label}`} active={myPos.includes(pos.id)} onPress={() => togglePos(pos.id)} />)}
            </View>
          </View>
        ))}
      </View>
      ...
      <View style={st.card}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <TouchableOpacity onPress={() => Alert.alert("Güvenilirlik", "Yoklamada 'katıldı' / 'gelmedi' kayıtlarından hesaplanır. İlk 5 maçta yüzde gösterilmez; sonrasında yumuşatılmış oran kullanılır. Maça 24 saatten az kala ayrılmak ya da iptal etmek de 'gelmedi' gibi işler.")}>
            <Text style={st.cardTitle}>GÜVENİLİRLİK <Text style={{ color: C.faint }}>ⓘ</Text></Text>
          </TouchableOpacity>
        ...
        {user.paymentStats && user.paymentStats.pct != null && (
          <Text style={{ fontSize: 12, color: C.faint, marginTop: 6 }}>
            💳 Ödeme düzeni: <Text style={{ fontWeight: "900", color: user.paymentStats.pct >= 80 ? C.pitch : C.kit }}>%{user.paymentStats.pct}</Text> ({user.paymentStats.paid} zamanında{user.paymentStats.late ? `, ${user.paymentStats.late} geç` : ""}{user.paymentStats.overdue ? `, ${user.paymentStats.overdue} bekliyor` : ""})
          </Text>
        )}
      ...
      <TouchableOpacity onPress={onBringTeam} style={[st.inviteRow, { borderColor: C.pitchSoft }]}>
        <View style={[st.settingsIcon, { backgroundColor: C.pitchSoft }]}>
          <Ionicons name="people-outline" size={18} color={C.turf} />
        </View>
        <View style={{ flex: 1 }}>
          <Text style={{ fontWeight: "800", fontSize: 14, color: C.ink }}>Ekibini getir</Text>
          <Text style={{ fontSize: 12, color: C.faint, marginTop: 2 }}>Haftalık maçını aç, WhatsApp grubuna davet linkini at — "kim var?" sayımı bitsin</Text>
        </View>
        <Ionicons name="chevron-forward" size={18} color={C.turf} />
      </TouchableOpacity>
