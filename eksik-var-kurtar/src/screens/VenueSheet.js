...
export default function VenueSheet({ cat = null, onCat = null, visible, onClose, cityName, categoryName, onList, onAdd, onPick }) {
  ...
  useEffect(() => { if (visible) { setMode("liste"); setQ(""); setPin(null); setAdi(""); setSecili(null); yukle(""); } }, [visible, yukle]);

  const sec = (v) => { onPick({ name: v.name, lat: v.lat ?? null, lng: v.lng ?? null, venueId: v.id || null }); onClose(); };
  const [secili, setSecili] = useState(null);           // listeden doğrulanan saha
  const [pinAdres, setPinAdres] = useState(null);       // null | "ariyor" | "yok" | metin
  const pinle = (k) => {
    setPin(k); setPinAdres("ariyor");
    fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${k.latitude}&lon=${k.longitude}&zoom=17&accept-language=tr`, { headers: { "User-Agent": "EksikVar/1.0" } })
      .then((r) => r.json())
      .then((js) => {
        const a2 = (js && js.address) || {};
        const parca = [a2.road || a2.pedestrian, a2.neighbourhood || a2.suburb || a2.quarter, a2.town || a2.city_district || a2.county].filter(Boolean);
        setPinAdres(parca.length ? parca.join(", ") : "yok");
      })
      .catch(() => setPinAdres("yok"));
  };
  const dogrula = (v) => {
    if (v.lat != null && v.lng != null) {
      setSecili(v); setAdi(v.name);
      pinle({ latitude: v.lat, longitude: v.lng });
      setMode("harita");
    } else sec(v);                                       // konumsuz kayıt: direkt seç
  };

  const kaydetVeSec = async () => {
    const ad = adi.trim();
    if (!ad || busy) return;
    if (secili) { sec({ ...secili, name: ad, lat: pin.latitude, lng: pin.longitude }); return; }
    setBusy(true);
    try {
  ...
  const region = pin ? { latitude: pin.latitude, longitude: pin.longitude, latitudeDelta: 0.01, longitudeDelta: 0.01 } : ilkKonumlu
        ...
        <View style={st.sheet}>
          ...
          {onCat && (
            <View style={{ flexDirection: "row", gap: 6, marginTop: 8 }}>
              {CATEGORIES.map((c) => (
                <TouchableOpacity key={c.id} onPress={() => onCat(c.id)}
                  style={{ flexDirection: "row", alignItems: "center", gap: 3, borderRadius: 999, borderWidth: 1,
                    borderColor: cat === c.id ? C.turf : C.line, backgroundColor: cat === c.id ? C.turf : C.chalk,
            ...
            <View style={{ flex: 1 }}>
              ...
              {loading ? <ActivityIndicator color={C.pitch} style={{ marginTop: 20 }} /> : (
                <FlatList
                  ...
                  style={{ flexGrow: 0 }}
                  renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => dogrula(item)} style={st.row}>
          ...
          {mode === "harita" && (
            <View style={{ flex: 1 }}>
              ...
              <MapView ref={mapRef} style={st.map} initialRegion={region}
                onPress={(e) => pinle(e.nativeEvent.coordinate)}>
                {pin && <Marker coordinate={pin} draggable onDragEnd={(e) => pinle(e.nativeEvent.coordinate)} />}
              </MapView>
              <Text style={{ fontSize: 12, color: C.faint, marginTop: 8 }}>
                {secili ? t("İğneyi sürükleyip konumu düzeltebilirsin 📍") : pin ? t("İğne bırakıldı — sahanın adını yazın:") : t("Sahanın olduğu noktaya dokunun.")}
              </Text>
              {pin && (
                <Text style={{ fontSize: 12, fontWeight: "800", color: C.pitch, marginTop: 4 }} numberOfLines={2}>
                  📍 {pinAdres === "ariyor" ? t("Adres bulunuyor…") : pinAdres === "yok" || !pinAdres ? t("Adres okunamadı — iğne yine de geçerli") : pinAdres}
                </Text>
              )}
              ...
              <View style={st.footerRow}>
                <TouchableOpacity onPress={() => { setSecili(null); setMode("liste"); }} style={[st.footBtn, { backgroundColor: C.chalk, borderWidth: 1, borderColor: C.line }]}>
                ...
                <TouchableOpacity disabled={!pin || !adi.trim() || busy} onPress={kaydetVeSec}
                  ...
                  <Text style={st.footBtnText}>{busy ? t("Kaydediliyor…") : secili ? t("Konum doğru — seç") : t("Kaydet ve seç")}</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
...
const mkSt = () => StyleSheet.create({
  ...
  map: { height: Math.round(Dimensions.get("window").height * 0.32), minHeight: 240, borderRadius: 14, marginTop: 4 },
