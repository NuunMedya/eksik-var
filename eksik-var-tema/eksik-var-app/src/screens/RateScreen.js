    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: C.chalk }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 140 }} keyboardShouldPersistTaps="handled">
        ...
        <View style={{ flexDirection: "row", gap: 8, marginBottom: 10 }}>
          <TouchableOpacity onPress={() => setRatings((r) => Object.fromEntries(Object.entries(r).map(([k, v]) => [k, { ...v, stars: 5 }])))} style={st.quick}><Text style={st.quickText}>Hepsine 5 ⭐</Text></TouchableOpacity>
          <TouchableOpacity onPress={() => setRatings((r) => Object.fromEntries(Object.entries(r).map(([k, v]) => [k, { ...v, stars: 4 }])))} style={st.quick}><Text style={st.quickText}>Hepsine 4</Text></TouchableOpacity>
          <TouchableOpacity onPress={() => setRatings((r) => Object.fromEntries(Object.entries(r).map(([k, v]) => [k, { ...v, stars: 0 }])))} style={st.quick}><Text style={st.quickText}>Temizle</Text></TouchableOpacity>
        </View>
        {mvpEnabled && (
...
const mkSt = () => StyleSheet.create({
  ...
  quickText: { fontSize: 12, fontWeight: "800", color: C.turf },
