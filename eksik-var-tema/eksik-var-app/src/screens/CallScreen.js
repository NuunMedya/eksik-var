  ...
  return (
    <View style={st.wrap}>
      <View style={{ alignItems: "center", marginTop: 60 }}>
        <Text style={st.kicker}>UYGULAMA İÇİ ARAMA</Text>
        ...
        <View style={st.privacy}>
          ...
          <Text style={{ color: C.mist, fontSize: 12 }}>Numaralar gizli · uçtan uca uygulama içi</Text>
        </View>
      </View>

      <View style={st.controls}>
        <TouchableOpacity onPress={() => setMuted((m) => !m)} style={[st.ctrl, muted && st.ctrlOn]}>
          <Ionicons name={muted ? "mic-off" : "mic"} size={26} color={muted ? C.turf : "#fff"} />
          <Text style={[st.ctrlLabel, muted && { color: C.turf }]}>{muted ? "Sessizde" : "Sessize al"}</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity onPress={() => setSpeaker((s) => !s)} style={[st.ctrl, speaker && st.ctrlOn]}>
          <Ionicons name={speaker ? "volume-high" : "volume-medium"} size={26} color={speaker ? C.turf : "#fff"} />
          <Text style={[st.ctrlLabel, speaker && { color: C.turf }]}>Hoparlör</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
