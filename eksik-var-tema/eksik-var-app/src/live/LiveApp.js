...
export default function LiveApp() {
  ...
  const shareEvent = async (ev) => { try { const r = await Share.share({ message: buildShareText(ev, me) }); if (r.action === Share.sharedAction) showToast("Paylaşıldı 🚀"); } catch { /* sessiz */ } };
  ...
  const checkInWithCode = async (eventId, code) => { try { await api.checkInWithCode(eventId, code); await refreshEvents(); showToast("Sahadayım kaydedildi ✓"); } catch (e) { fail(e); } };
  useEffect(() => {
          ...
          {!hideTabs && (
            <View style={st.tabbar}>
              <TouchableOpacity style={st.tabBtn} onPress={() => { setTab("home"); setView({ name: "root" }); }}>
                ...
                <Text style={[st.tabLabel, { color: tab === "home" ? C.turf : C.gray }]}>{t("tab.home")}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={st.tabBtn} onPress={() => { setTab("search"); setView({ name: "root" }); }}>
                ...
                <Text style={[st.tabLabel, { color: tab === "search" ? C.turf : C.gray }]}>{t("tab.search")}</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity style={st.tabBtn} onPress={() => { setTab("chats"); setView({ name: "root" }); }}>
                ...
                <Text style={[st.tabLabel, { color: tab === "chats" ? C.turf : C.gray }]}>{t("tab.chats")}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={st.tabBtn} onPress={() => { setTab("profile"); setView({ name: "root" }); }}>
                ...
                <Text style={[st.tabLabel, { color: tab === "profile" ? C.turf : C.gray }]}>{t("tab.profile")}</Text>
              </TouchableOpacity>
            </View>
          )}
