      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 60 }}>
        ...
        {event && (
          <TouchableOpacity onPress={() => onOpenEvent(event.id)} style={st.eventCard}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 11, fontWeight: "900", letterSpacing: 0.8, color: C.turf }}>BAĞLI ETKİNLİK</Text>
        ...
        {onLineup && members.length >= 4 && (
          <TouchableOpacity onPress={onLineup} style={st.lineupBtn}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontWeight: "900", fontSize: 14, color: C.ink }}>Kura çek</Text>
              <Text style={{ fontSize: 12, color: C.faint, marginTop: 1 }}>Yelekliler – yeleksizler: puana göre dengeli iki takım</Text>
            </View>
            <Ionicons name="chevron-forward" size={18} color={C.gray} />
          </TouchableOpacity>
        )}
        ...
        {season && season.length > 0 && (
          <>
            <Text style={st.section}>SEZON TABLOSU</Text>
            <View style={st.card}>
              <View style={[st.memberRow, { paddingVertical: 6 }]}>
                <Text style={{ flex: 1, fontSize: 11, fontWeight: "900", color: C.faint }}>OYUNCU</Text>
              ...
              {season.slice(0, 8).map((r, i) => (
                <View key={r.id} style={[st.memberRow, i === Math.min(season.length, 8) - 1 && { borderBottomWidth: 0 }]}>
                  <Text style={{ flex: 1, fontSize: 13, fontWeight: i === 0 ? "900" : "700", color: C.ink }} numberOfLines={1}>{i === 0 ? "👑 " : ""}{r.name}{r.guest ? " ·" : ""}{r.guest ? <Text style={{ color: C.faint, fontWeight: "600" }}> misafir</Text> : null}</Text>
