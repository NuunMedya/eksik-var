...
declare
  ...
  v_conv bigint; v_app uuid; v_app2 uuid; v_poll uuid; v_post uuid; v_team uuid; v_offer uuid;
...
begin
  ...
  -- ==== takım + teklif makinesi ====
  perform pg_temp.ol(veli);
  insert into public.teams (owner_id, name, city_id) values (veli, 'Veli FC', 6);
  select id into v_team from public.teams where owner_id = veli limit 1;
  if (select count(*) from public.team_members where team_id = v_team and user_id = veli and role = 'kaptan') <> 1 then
    raise exception 'kaptan otomatik eklenmedi';
  end if;
  insert into public.offers (kind, from_user, to_user, team_id, message) values ('takim', veli, ali, v_team, 'Gel bize');
  select id into v_offer from public.offers where from_user = veli order by created_at desc limit 1;
  perform pg_temp.ol(deli);
  begin
    update public.offers set status = 'kabul' where id = v_offer;
    if exists (select 1 from public.offers where id = v_offer and status = 'kabul') then
      raise exception 'BEKLENEN_HATA_YOK: yabanci_karar';
    end if;
  ...
  update public.offers set status = 'kabul' where id = v_offer;
  perform pg_temp.yonetici();
  if (select count(*) from public.team_members where team_id = v_team and user_id = ali) <> 1 then
    raise exception 'kabul kadroya yazmadı';
  end if;
  if (select count(*) from public.notifications where user_id = ali and type = 'teklif') < 1 then
    raise exception 'teklif bildirimi düşmedi';
  end if;
  perform pg_temp.gec('takım/teklif: kaptan otomatik, karar yalnız muhataba, kabul kadroya işler');

  -- ==== keşfet paylaşımları ====
  perform pg_temp.ol(veli);
  insert into public.posts (user_id, city_id, caption) values (veli, 6, 'İlk paylaşımım!');
  select id into v_post from public.posts where user_id = veli order by created_at desc limit 1;
  insert into public.post_likes (post_id, user_id) values (v_post, veli);
  perform pg_temp.ol(deli);
  begin
    insert into public.posts (user_id, city_id, caption) values (ali, 6, 'Sahte gönderi');
    raise exception 'BEKLENEN_HATA_YOK: post_sahte';
  exception when insufficient_privilege then null;
  end;
  perform pg_temp.ol(ali);
  begin
    delete from public.posts where id = v_post;
    if found then raise exception 'BEKLENEN_HATA_YOK: post_baskasi_sildi'; end if;
  end;
  perform pg_temp.yonetici();
  if (select count(*) from public.post_likes where post_id = v_post) <> 1 then
    raise exception 'beğeni yazılamadı';
  end if;
  perform pg_temp.gec('keşfet: paylaşım kendi adına, silme yalnız sahibine, beğeni işliyor');

  -- ==== transfer pazarı ====
  perform pg_temp.ol(veli);
  insert into public.player_listings (user_id, category_id, city_id, positions, bio)
  values (veli, 1, 6, array['forvet'], 'Deneme vitrin') on conflict (user_id) do update set bio = excluded.bio;
  begin
    insert into public.player_listings (user_id, category_id, city_id) values (ali, 1, 6);
    raise exception 'BEKLENEN_HATA_YOK: pazar_sahte';
  exception when insufficient_privilege then null;
  end;
  perform pg_temp.ol(ali);
  if (select count(*) from public.player_listings where user_id = veli) <> 1 then
    raise exception 'pazar ilanı okunamadı';
  end if;
  perform pg_temp.gec('pazar: vitrin kendi adına açılır, başkası adına açılamaz, aktifler okunur');
