...
import { contactRules, DEFAULT_SETTINGS, buildShareText, buildInviteText, buildTeamInvite, extractIban, guestPlayers, buildMatchSummary, REPORT_REASONS, memberFromOrg, openPositions, APP_LINK } from "../data";
...
export default function LiveApp() {
  ...
  const hideTabs = ["chat", "create", "settings", "groupInfo", "attendance", "notifications", "rate", "blocked", "profile", "lineup", "editProfile", "penalti", "kadroDene", "basket", "voleybol", "tenis"].includes(view.name);
  const ustEkran = ["hub", "penalti", "kadroDene", "basket", "voleybol", "tenis"].includes(view.name) || !!activeEvent || !!call;
          ...
          {view.name === "profile" && profile && (
            <View style={StyleSheet.absoluteFill}>
              <UserProfileScreen user={profile.user} comments={profile.comments} loading={profile.loading}
                ...
                onTeamInvite={profile.user ? async () => { try {
                  const id = await api.openDirectChat(meId, profile.user.id);
                  await api.sendMessage(meId, id, t("🏆 Takım daveti: Merhaba {p0}! Seni takımımızda görmek istiyoruz — katılır mısın?", { p0: profile.user.name.split(" ")[0] }) + `\n— ${user.name} · 🔗 ${APP_LINK}`);
                  await refreshChats(); openChat(id);
                } catch (e) { fail(e); } } : null}
