...
t("penaltı mantığı", () => {
  assert.equal(pn.sut("sol", "sag").gol, true);
  assert.equal(pn.sut("orta", "orta").gol, false);
  assert.ok(pn.KOSELER.includes(pn.kaleciSec(() => 0.99)));
  assert.equal(pn.kaleciSec(() => 0), "sol");
});
console.log(`\n${n} test geçti`);
