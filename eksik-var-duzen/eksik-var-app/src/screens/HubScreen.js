...
const SATIRLAR = [
  { key: "takim",   emoji: "🛡", renk: "#E0E7FF", title: "Takımım",        desc: "Kadron, amblemin, davetlerin" },
...
const KARELER = [
  { key: "create",    emoji: "⚡", renk: "#D1FAE5", title: "Maç Kur" },
  { key: "pazar",     emoji: "🔁", renk: "#FFEDD5", title: "Transfer" },
  { key: "teklifler", emoji: "💌", renk: "#FCE7F3", title: "Teklifler" },
  { key: "kadro",     emoji: "📋", renk: "#DBEAFE", title: "Kadro Dene" },
];
const OYUNLAR = [
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 130 }}>
        <Text style={st.bolum}>{t("MAÇ & OYUNCU")}</Text>
        <View style={st.kareIzgara}>
          {KARELER.map((k) => (
            <TouchableOpacity key={k.key} onPress={() => onGo(k.key)} activeOpacity={0.85} style={st.kare}>
              <View style={[st.kareRozet, { backgroundColor: k.renk }]}><Text style={{ fontSize: 22 }}>{k.emoji}</Text></View>
              <Text style={st.kareBaslik} numberOfLines={1}>{t(k.title)}</Text>
            </TouchableOpacity>
          ))}
        </View>

        <Text style={st.bolum}>{t("TAKIM & TOPLULUK")}</Text>
        {SATIRLAR.map((k) => (
        ...
        <Text style={st.bolum}>🎮 {t("OYUN SALONU")}</Text>
