...
import { Linking, Modal, ScrollView, TextInput, KeyboardAvoidingView, Platform } from "react-native";
import { t, onLangChange } from "../i18n";
import { saveTheme, saveLang } from "../prefs";
import { Toast, PickerSheet, Avatar } from "../components";
import { OfferSheet } from "../screens/sheets";
import { Alert } from "react-native";
import * as api from "./api";
import { DEFAULT_SPONSORS } from "../sponsors";
import { cloneForRepeat, CATEGORIES, POSITIONS, GUNLER, LEVELS } from "../data";
...
export default function LiveApp() {
  ...
  const teklifYolla = async (metin, ilan) => {
    const p = teklifKisi; setTeklifKisi(null);
    if (!p) return;
    try {
      const govde = ilan ? "\u{1F3AF} " + ilan.title + " \u00B7 " + ilan.date + " " + ilan.time + "\n" + metin : metin;
      await api.createOffer(meId, { kind: "oyuncu", toUser: p.userId, message: govde });
  ...
  const vitrinHazirla = (f) => {
    setVitrinYazi((f.bio || "").trim()); setVitrinUcret("");
    const kat = f.cat || 1;
    setVitrinCat(kat);
    const gecerli = new Set(((POSITIONS[kat] || []).map((p2) => p2.id)));
    setVitrinPos(((f.positions && f.positions.length ? f.positions : me.positions) || []).filter((x) => gecerli.has(x)));
    setVitrinGun(f.days || []);
    setVitrinSeviye((me.catLevels && me.catLevels[kat]) || "Farketmez");
    setVitrinIlce(me.district || "");
    setVitrinMedya(null);
    setVitrinOniz(f);
  };
  const saveListing = async (f) => { try {
    await api.upsertListing(meId, user, f); await refreshMarket();
    showToast(t("Kaydedildi ✓ — vitrin kartını + Paylaş'tan basabilirsin 🎯"));
  } catch (e) { fail(e); } };
  const vitrinAtolyeAc = () => {
    const kayit = myListing && myListing.active ? myListing : null;
    vitrinHazirla(kayit
      ? { cat: kayit.cat, positions: kayit.positions || [], days: kayit.days || [], bio: kayit.bio || "" }
      : { cat: (me.favCats && me.favCats[0]) || 1, positions: me.positions || [], days: [], bio: "" });
  };
  const [vitrinCat, setVitrinCat] = useState(1);
  const [vitrinPos, setVitrinPos] = useState([]);
  const [vitrinGun, setVitrinGun] = useState([]);
  const [vitrinSeviye, setVitrinSeviye] = useState("Farketmez");
  const [vitrinIlce, setVitrinIlce] = useState("");
  const [ilceSecici, setIlceSecici] = useState(false);
  const [vitrinMedya, setVitrinMedya] = useState(null);
  const vitrinMedyaSec = async () => {
    try {
      ...
      if (!r.canceled && r.assets && r.assets[0]) {
        const a2 = r.assets[0];
        setVitrinMedya({ uri: a2.uri, video: a2.type === "video" });
      }
    } catch {}
  };
  const [vitrinOniz, setVitrinOniz] = useState(null);
  const [vitrinYazi, setVitrinYazi] = useState("");
  const [vitrinUcret, setVitrinUcret] = useState("");
  const vitrinPaylas = async () => {
    setVitrinOniz(null);
    try {
      const govde = (vitrinYazi || "").trim() || t("🎯 Vitrindeyim — kadronda eksik varsa hazırım!");
      const ucretSatir = vitrinUcret.trim() ? "\n\u{1F4B0} " + t("Maç başı") + " " + vitrinUcret.trim() + "\u20BA" : "";
      await api.upsertListing(meId, user, { cat: vitrinCat, positions: vitrinPos, days: vitrinGun, bio: (vitrinYazi || "").trim() });
      const yeniSeviyeler = { ...(me.catLevels || {}), [vitrinCat]: vitrinSeviye };
      await api.updateIdentity(meId, { favCats: me.favCats || [], positions: me.positions || [], levels: yeniSeviyeler });
      setMe((u) => (u ? { ...u, catLevels: yeniSeviyeler } : u));
      if (vitrinIlce && vitrinIlce !== me.district) {
        try { await api.setDistrict(meId, user.city, vitrinIlce); setMe((u) => (u ? { ...u, district: vitrinIlce } : u)); } catch { /* sessiz */ }
      }
      await refreshMarket();
      await api.createPost(meId, me, { caption: govde + ucretSatir, attach: true,
        imageUri: vitrinMedya && !vitrinMedya.video ? vitrinMedya.uri : null,
        videoUri: vitrinMedya && vitrinMedya.video ? vitrinMedya.uri : null });
      await refreshPosts();
      showToast(t("Keşfet'te yayında — 🎯 Oyuncu vitrini süzgecinde"));
    } catch (e) { fail(e); }
  };
  ...
  useEffect(() => {
    ...
    const unsub = api.subscribe({
      onChangeCalls: (p) => {
        const r = p && p.new;
        if (!r) return;
        if (p.eventType === "INSERT" && r.callee_id === meId && !r.ended_at) {
          const tumu = [];
          chats.forEach((c) => { if (c.other) tumu.push(c.other); (c.members || []).forEach((m) => tumu.push(m)); });
          const kisi = tumu.find((u) => u.id === r.caller_id);
          setCall({ callId: r.id, live: true, status: "gelen", name: (kisi && kisi.name) || t("Biri"), avatar: kisi && kisi.avatar });
        } else if (r.caller_id === meId || r.callee_id === meId) {
          setCall((c) => {
  ...
  const createEvent = async (f) => {
    ...
    try {
      ...
      if (f.cat && !((me.favCats || []).includes(f.cat))) {
        const yeniBrans = [...(me.favCats || []), f.cat];
        try {
          await api.updateIdentity(meId, { favCats: yeniBrans, positions: me.positions || [], levels: me.catLevels || {} });
          setMe((u) => (u ? { ...u, favCats: yeniBrans } : u));
        } catch { /* sessiz */ }
      }
  ...
  const startCall = async (c) => {
    const hedef = c && (c.otherId || (c.other && c.other.id));
  ...
  const callMember = async (kisi) => { try {
    const conv = await api.openDirectChat(meId, kisi.id);
    const row = await api.logCall(meId, kisi.id);
    setCall({ callId: row.id, live: true, status: "aranıyor", name: kisi.name, avatar: kisi.avatar, convId: conv && conv.id });
  } catch (e) { fail(e); } };
  ...
  const activeChat = view.name === "chat" ? chats.find((c) => String(c.id) === String(view.id)) : null;
  const infoChat = view.name === "groupInfo" ? chats.find((c) => String(c.id) === String(view.id)) : null;
        ...
        <View style={{ flex: 1, backgroundColor: C.chalk }}>
          {tab === "home" && !ustEkran && view.name !== "event" && (
            <HomeScreen user={user} events={events} onOpen={(id) => setView({ name: "event", id })}
              ...
              key={homeKind} initialKind={homeKind} onMatches={() => setView({ name: "maclarim" })} onOpenShowcase={() => { setTab("search"); vitrinAtolyeAc(); }}
          ...
          {tab === "search" && !ustEkran && (
            <SearchScreen onSearchUsers={searchUsers} onSearchEvents={searchEvents} suggestions={suggestions}
              ...
              posts={posts} onLikePost={likePost} onDeletePost={removePost} onCompose={vitrinAtolyeAc} onOfferPost={teklifGonder} onMessagePost={async (p) => { try { const id = await api.openDirectChat(meId, p.userId); await refreshChats(); openChat(id); } catch (e) { fail(e); } }} cityName={user.city} />
          )}
          ...
          {tab === "chats" && view.name === "chat" && activeChat && (
            <ChatRoomScreen chat={activeChat} apps={apps} rules={rulesFor(activeChat)} onCall={() => startCall(activeChat)}
              onSendVoice={async (cid, uri, sec) => { try { await api.sendVoiceMessage(meId, cid, uri, sec); } catch (e) { fail(e); } }}
          ...
          {view.name === "takim" && <TakimScreen team={myTeamRow} members={takimUyeler} meId={meId}
            ...
            onGoMarket={() => { setTab("search"); setView({ name: "root" }); }}
          ...
          {vitrinOniz && (
            <Modal visible animationType="slide" onRequestClose={() => setVitrinOniz(null)}>
              <KeyboardAvoidingView style={{ flex: 1, backgroundColor: C.chalk }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
                <View style={{ backgroundColor: C.turf, paddingTop: 56, paddingBottom: 11, alignItems: "center" }}>
                  <Text style={{ color: "#fff", fontWeight: "900", fontSize: 13, letterSpacing: 1 }}>{"\u{1F3AF} "}{t("VİTRİN GÖNDERİN — ÖNİZLEME")}</Text>
                </View>
                <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 140 }} keyboardShouldPersistTaps="handled">
                  <View style={{ backgroundColor: C.surface, borderRadius: 20, padding: 16, shadowColor: "#000", shadowOpacity: 0.1, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 3 }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                      <Avatar name={me.name} uri={me.avatar} size={54} />
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontWeight: "900", fontSize: 18, color: C.ink }}>{me.name}</Text>
                        <TouchableOpacity onPress={() => setIlceSecici(true)} style={{ flexDirection: "row", alignItems: "center", gap: 3 }}>
                          <Text style={{ fontSize: 12.5, color: C.faint }}>{"\u{1F4CD} "}{user.city} · </Text>
                          <Text style={{ fontSize: 12.5, fontWeight: "800", color: vitrinIlce ? C.turfText : C.kit }}>{vitrinIlce || t("İlçe seç")}</Text>
                          <Ionicons name="chevron-down" size={12} color={C.faint} />
                        </TouchableOpacity>
                      </View>
                      <Text style={{ fontSize: 30 }}>{(CATEGORIES.find((k) => k.id === vitrinCat) || {}).icon || "\u26BD"}</Text>
                    </View>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 12 }} contentContainerStyle={{ gap: 7 }}>
                      {CATEGORIES.map((k) => (
                        <TouchableOpacity key={k.id} onPress={() => { setVitrinCat(k.id); const g2 = new Set((POSITIONS[k.id] || []).map((p2) => p2.id)); setVitrinPos((eski) => eski.filter((x) => g2.has(x))); setVitrinSeviye((me.catLevels && me.catLevels[k.id]) || "Farketmez"); }}
                          style={{ borderRadius: 999, paddingHorizontal: 11, paddingVertical: 6, backgroundColor: vitrinCat === k.id ? C.turf : C.chalk, borderWidth: 1, borderColor: vitrinCat === k.id ? C.turf : C.line }}>
                          <Text style={{ fontSize: 12, fontWeight: "800", color: vitrinCat === k.id ? "#fff" : C.ink }}>{k.icon} {t(k.name)}</Text>
                        </TouchableOpacity>
                      ))}
                    </ScrollView>
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 12 }}>
                      {(POSITIONS[vitrinCat] || []).map((p2) => {
                        const sec = vitrinPos.includes(p2.id);
                        return (
                          <TouchableOpacity key={p2.id} onPress={() => setVitrinPos((l) => sec ? l.filter((x) => x !== p2.id) : [...l, p2.id])}
                            style={{ backgroundColor: sec ? C.turf : C.pitchSoft, borderRadius: 999, paddingHorizontal: 11, paddingVertical: 6 }}>
                            <Text style={{ fontSize: 12.5, fontWeight: "800", color: sec ? "#fff" : C.turfText }}>{p2.icon} {t(p2.label)}</Text>
                          </TouchableOpacity>
                        );
                      })}
                    </View>
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 12, alignItems: "center" }}>
                      <Text style={{ fontSize: 12.5, color: C.faint }}>{"\u{1F3C5} "}{t("Seviye")}:</Text>
                      {LEVELS.map((lv) => {
                        const sec = vitrinSeviye === lv;
                        return (
                          <TouchableOpacity key={lv} onPress={() => setVitrinSeviye(lv)}
                            style={{ backgroundColor: sec ? C.kit : C.chalk, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 5, borderWidth: 1, borderColor: sec ? C.kit : C.line }}>
                            <Text style={{ fontSize: 11.5, fontWeight: "800", color: sec ? "#fff" : C.ink }}>{t(lv)}</Text>
                          </TouchableOpacity>
                        );
                      })}
                    </View>
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 12, alignItems: "center" }}>
                      <Text style={{ fontSize: 12.5, color: C.faint }}>{"\u{1F4C5} "}{t("Müsait")}:</Text>
                      {[1, 2, 3, 4, 5, 6, 0].map((i3) => { const g3 = GUNLER[i3];
                        const sec = vitrinGun.includes(i3);
                        return (
                          <TouchableOpacity key={g3} onPress={() => setVitrinGun((l) => sec ? l.filter((x) => x !== i3) : [...l, i3].sort())}
                            style={{ backgroundColor: sec ? C.turfText : C.chalk, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 5, borderWidth: 1, borderColor: sec ? C.turfText : C.line }}>
                            <Text style={{ fontSize: 11.5, fontWeight: "800", color: sec ? (C.isDark ? C.turf : "#fff") : C.ink }}>{g3}</Text>
                          </TouchableOpacity>
                        );
                      })}
                    </View>
                    {vitrinMedya ? (
                      <View style={{ marginTop: 12, borderRadius: 14, overflow: "hidden" }}>
                        {vitrinMedya.video ? (
                          <View style={{ height: 160, backgroundColor: "#000", alignItems: "center", justifyContent: "center" }}>
                            <Text style={{ fontSize: 34 }}>{"\u{1F3AC}"}</Text>
                            <Text style={{ color: "#fff", fontSize: 12, fontWeight: "800", marginTop: 4 }}>{t("Video eklendi")}</Text>
                          </View>
                        ) : (
                          <Image source={{ uri: vitrinMedya.uri }} style={{ height: 200, width: "100%" }} resizeMode="cover" />
                        )}
                        <TouchableOpacity onPress={() => setVitrinMedya(null)} style={{ position: "absolute", top: 8, right: 8, backgroundColor: "rgba(0,0,0,0.55)", borderRadius: 999, padding: 6 }}>
                          <Ionicons name="close" size={16} color="#fff" />
                        </TouchableOpacity>
                      </View>
                    ) : (
                      <TouchableOpacity onPress={vitrinMedyaSec} style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 12, backgroundColor: C.chalk, borderWidth: 1, borderColor: C.line, borderStyle: "dashed", borderRadius: 14, paddingVertical: 14 }}>
                        <Ionicons name="image-outline" size={19} color={C.turfText} />
                        <Text style={{ fontSize: 13, fontWeight: "800", color: C.turfText }}>{t("Fotoğraf ya da video ekle")}</Text>
                      </TouchableOpacity>
                    )}
                    <TextInput value={vitrinYazi} onChangeText={setVitrinYazi} multiline maxLength={200}
                      placeholder={t("🎯 Vitrindeyim — kadronda eksik varsa hazırım!")} placeholderTextColor={C.placeholder}
                      style={{ marginTop: 12, minHeight: 84, textAlignVertical: "top", backgroundColor: C.chalk, borderWidth: 1, borderColor: C.line, borderRadius: 14, padding: 12, fontSize: 14.5, color: C.ink, lineHeight: 21 }} />
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 10 }}>
                      <Text style={{ fontSize: 17 }}>{"\u{1F4B0}"}</Text>
                      <TextInput value={vitrinUcret} onChangeText={(x) => setVitrinUcret(x.replace(/[^0-9]/g, ""))} keyboardType="number-pad" maxLength={5}
                        placeholder={t("Ücret beklentin (\u20BA, opsiyonel)")} placeholderTextColor={C.placeholder}
                        style={{ flex: 1, backgroundColor: C.chalk, borderWidth: 1, borderColor: C.line, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 9, fontSize: 14, color: C.ink }} />
                      {!!vitrinUcret && <Text style={{ fontSize: 13, fontWeight: "900", color: C.ink }}>{t("Maç başı")} {vitrinUcret}{"\u20BA"}</Text>}
                    </View>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 14, backgroundColor: C.pitchSoft, borderRadius: 12, padding: 10 }}>
                      <Text style={{ fontSize: 15 }}>{"\u{1F48C}"}</Text>
                      <Text style={{ flex: 1, fontSize: 12, color: C.turfText, fontWeight: "700" }}>{t("İnsanlar bu karttan sana transfer teklifi atabilir — kabul edersen sohbet açılır.")}</Text>
                    </View>
                  </View>
                </ScrollView>
                <View style={{ flexDirection: "row", gap: 9, padding: 14, paddingBottom: 30, backgroundColor: C.surface, borderTopWidth: 1, borderTopColor: C.line }}>
                  <TouchableOpacity onPress={() => setVitrinOniz(null)} style={{ flex: 1, backgroundColor: C.chalk, borderRadius: 14, paddingVertical: 13, alignItems: "center" }}>
                    <Text style={{ fontSize: 13.5, fontWeight: "800", color: C.faint }}>{t("Vazgeç")}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={vitrinPaylas} style={{ flex: 1.4, backgroundColor: C.kit, borderRadius: 14, paddingVertical: 13, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 6 }}>
                    <Text style={{ fontSize: 14 }}>{"\u{1F4E3}"}</Text>
                    <Text style={{ fontSize: 13.5, fontWeight: "900", color: "#fff" }}>{t("Keşfette paylaş")}</Text>
                  </TouchableOpacity>
                </View>
              </KeyboardAvoidingView>
              <PickerSheet visible={ilceSecici} title={t("İlçe seç")} items={ilceleri(user.city)} value={vitrinIlce}
                onSelect={(x) => { setVitrinIlce(typeof x === "string" ? x : x.label); setIlceSecici(false); }} onClose={() => setIlceSecici(false)} />
            </Modal>
          )}
          <OfferSheet visible={!!teklifKisi} kisi={teklifKisi} onClose={() => setTeklifKisi(null)} onSend={teklifYolla} myEvents={events.filter((e) => e.mine && !e.ended && e.status !== "iptal" && e.status !== "tamamlandi")} />
          ...
          {!hideTabs && (
            <View style={st.tabbar}>
              ...
              <TouchableOpacity style={[st.fab, { overflow: "hidden", backgroundColor: "#123B26" }]} onPress={() => setView({ name: view.name === "hub" ? "root" : "hub" })}>
                <Image source={require("../../assets/logo.png")} style={{ width: "132%", height: "132%" }} resizeMode="cover" />
              </TouchableOpacity>
