...
/* ---------- Başvuru ---------- */
export function ApplySheet({ visible, ev, onClose, onSend, myPositions = [] }) {
  const [note, setNote] = useState("");
  const [position, setPosition] = useState(null);
  if (!ev) return null;
  const slots = openPositions(ev);
  const hasChoice = slots.length > 0 && !(slots.length === 1 && slots[0].id === "farketmez");
    ...
    <SheetShell visible={visible} onClose={onClose}>
      ...
      {hasChoice && (
        <View style={{ marginTop: 10 }}>
          <Text style={st.label}>HANGİ MEVKİYE GELİYORSUN?</Text>
            ...
            {slots.map((sl) => (
              <TouchableOpacity key={sl.id} onPress={() => setPosition(sl.id)}
                ...
                <Text style={{ fontSize: 13, fontWeight: "800", color: chosen === sl.id ? "#fff" : C.ink }}>
                  {sl.icon} {sl.label} · {sl.quota - sl.filled} yer
                </Text>
              </TouchableOpacity>
            ))}
          </View>
      ...
      <TouchableOpacity
        style={st.cta}
        onPress={() => {
          onSend(note.trim() || "Merhaba, ben varım! Detayları konuşabilir miyiz?", hasChoice ? chosen : null);
