...
/* ---------- Anket oluştur ---------- */
export function OfferSheet({ visible, kisi, onClose, onSend, myEvents = [] }) {
  const [metin, setMetin] = React.useState("");
  const [secili, setSecili] = React.useState(null);
  React.useEffect(() => { if (visible) { setMetin(""); setSecili(null); } }, [visible]);
        ...
        <TouchableOpacity activeOpacity={1} style={{ backgroundColor: C.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 18, paddingBottom: 34 }}>
          ...
          {myEvents.length > 0 && (
            <View style={{ marginBottom: 10 }}>
              <Text style={{ fontSize: 11, fontWeight: "900", letterSpacing: 0.6, color: C.faint, marginBottom: 6 }}>{"\u{1F4CB} "}{t("BİR İLANINA DAVET ET (İSTEĞE BAĞLI)")}</Text>
              {myEvents.slice(0, 4).map((ev) => {
                const sec = secili && secili.id === ev.id;
                return (
                  <TouchableOpacity key={ev.id} onPress={() => setSecili(sec ? null : ev)}
                    style={{ flexDirection: "row", alignItems: "center", gap: 8, borderWidth: 1.5, borderColor: sec ? C.turf : C.line, backgroundColor: sec ? C.pitchSoft : C.chalk, borderRadius: 12, paddingHorizontal: 11, paddingVertical: 9, marginBottom: 6 }}>
                    <Text style={{ fontSize: 14 }}>{sec ? "\u2705" : "\u{1F4CB}"}</Text>
                    <Text style={{ flex: 1, fontSize: 13, fontWeight: "800", color: C.ink }} numberOfLines={1}>{ev.title} · {ev.date} {ev.time}</Text>
                    {ev.missing > 0 && <Text style={{ fontSize: 11.5, fontWeight: "900", color: C.kit }}>{ev.missing} {t("eksik")}</Text>}
                  </TouchableOpacity>
                );
              })}
            </View>
          ...
          <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
            ...
            <TouchableOpacity onPress={() => onSend((metin || "").trim() || sablon, secili)} style={{ flex: 1.4, backgroundColor: C.kit, borderRadius: 14, paddingVertical: 13, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 6 }}>
