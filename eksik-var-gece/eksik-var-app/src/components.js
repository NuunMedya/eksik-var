...
import { View, Text, TouchableOpacity, TextInput, Modal, FlatList, Image, StyleSheet, Platform } from "react-native";
...
export const BACK_ICON = Platform.OS === "android" ? "arrow-back" : "chevron-back";

export function BackBtn({ onPress, light = false }) {
  return (
    <TouchableOpacity onPress={onPress} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
      style={{ width: 36, height: 36, borderRadius: 18, alignItems: "center", justifyContent: "center",
        backgroundColor: light ? "rgba(255,255,255,0.16)" : C.chalk }}>
      <Ionicons name={Platform.OS === "android" ? "arrow-back" : "chevron-back"}
        size={20} color={light ? "#fff" : C.ink} />
    </TouchableOpacity>
  );
}
  ...
  return (
    <TouchableOpacity onPress={onPress} style={{ alignSelf: "center" }}>
      ...
      <View style={[st.camBadge, light && { backgroundColor: C.surface, borderColor: C.turf }]}>
        <Ionicons name="camera" size={13} color={light ? C.turfText : "#fff"} />
      </View>
    </TouchableOpacity>
  );
}
  ...
  if (ev.status === "iptal")
    return (
      <View style={[st.badge, { backgroundColor: C.gray }]}>
        <Text style={st.badgeText}>{t("İptal edildi")}</Text>
      </View>
    );
  if (ev.status === "tamamlandi")
    return (
      <View style={[st.badge, { backgroundColor: C.turf }]}>
        <Text style={st.badgeText}>{t("Tamamlandı ✓")}</Text>
      </View>
    );
  if (ev.ended)
    return (
      <View style={[st.badge, { backgroundColor: C.kit }]}>
        <Text style={st.badgeText}>{t("Yoklama bekliyor")}</Text>
      </View>
    );
  if (ev.joined)
    return (
      <View style={[st.badge, { backgroundColor: C.pitch }]}>
        <Text style={st.badgeText}>{t("Kadrodasın ✓")}</Text>
      </View>
    );
  if (ev.status === "doldu" || remaining <= 0)
    return (
      <View style={[st.badge, { backgroundColor: C.turf }]}>
        <Text style={st.badgeText}>{t("Kadro tamam")}</Text>
      </View>
    );
  return (
    <View style={[st.badge, { backgroundColor: C.kit, flexDirection: "row", alignItems: "center", gap: 4 }]}>
      ...
      <Text style={st.badgeText}>{t("eksik")}</Text>
    </View>
  );
}
  ...
  return (
    <TouchableOpacity
      ...
      style={{
        ...
        backgroundColor: active ? C.turfText : C.surface,
        borderColor: active ? C.turf : C.line,
      }}
    >
      <Text style={{ fontSize: 13, fontWeight: "700", color: active ? (C.isDark ? C.turf : "#fff") : C.ink }}>
        {label}
      </Text>
    </TouchableOpacity>
  );
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                ...
                >
                  <Text style={{ flex: 1, fontSize: 15, fontWeight: active ? "800" : "600", color: active ? C.turfText : C.ink }}>
                    {label}
                  </Text>
            ...
            ListEmptyComponent={<Text style={{ textAlign: "center", color: C.faint, padding: 20 }}>{t("Sonuç yok")}</Text>}
          />
        </View>
      </View>
    </Modal>
