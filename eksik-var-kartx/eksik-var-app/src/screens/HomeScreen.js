...
export default function HomeScreen({ user, events, onOpen, onAttendance, onChangeCity, onNotifications, unreadCount = 0, blockedIds = [], onCreate = () => {}, onBringTeam = () => {}, loading = false, offline = false, sponsors = [], onSponsor = () => {}, market = [], onOpenPlayer = () => {}, onEditListing = () => {}, onOfferPlayer = null, initialKind = "oyuncu" , onCreateRakip = () => {} }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      {kind !== "oyuncu" && (
        <TouchableOpacity onPress={() => setKind("oyuncu")} style={{ paddingHorizontal: 18, paddingTop: 10 }}>
          ...
          kind === "oyuncu" && upcomingMine.length > 0 ? (
            ...
            {kind === "oyuncu" && (
              <TouchableOpacity onPress={() => setKind("pazar")} activeOpacity={0.85}
                style={{ flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "#F5F0FF", borderWidth: 1.5, borderColor: "#7C3AED", borderRadius: 16, padding: 13, marginBottom: 14, marginTop: -4 }}>
                <Text style={{ fontSize: 20 }}>🔁</Text>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 14, fontWeight: "900", color: "#6D28D9" }}>{t("Transfer Pazarı")}</Text>
