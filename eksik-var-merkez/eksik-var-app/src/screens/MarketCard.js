  ...
  return (
    <TouchableOpacity onPress={onOpen} activeOpacity={0.85} style={st.card}>
      ...
      {(p.positions || []).length > 0 && (
        <Text style={{ fontSize: 12, fontWeight: "800", color: C.turfText, marginTop: 8 }}>
          {p.positions.map((id) => posLabel(id)).join(" · ")}
        </Text>
      )}
      {(p.days || []).length > 0 && (
      ...
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "flex-end", gap: 14, marginTop: 10 }}>
        {onOffer && p.userId !== "me" && (
          <TouchableOpacity onPress={() => onOffer(p)} style={{ flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: C.kitSoft, borderRadius: 999, paddingHorizontal: 11, paddingVertical: 6 }}>
            <Text style={{ fontSize: 12, fontWeight: "900", color: C.kit }}>💌 {t("Teklif gönder")}</Text>
          </TouchableOpacity>
        )}
        <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
          <Text style={{ fontSize: 12, fontWeight: "900", color: C.pitch }}>{t("Profili gör")}</Text>
          <Ionicons name="chevron-forward" size={14} color={C.pitch} />
        </View>
      </View>
    </TouchableOpacity>
  );
