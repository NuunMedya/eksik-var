...
const USER_COLS = "id, full_name, username, avatar_url, rating_avg, rating_count, reliability_pct, events_joined, events_organized, no_show_count, city_id, district_id, is_verified, status, status_reason, suspended_until, contact_mode, contact_scope, quiet_enabled, quiet_start, quiet_end, notif_basvuru, notif_mesaj, notif_hatirlatma, cities(name), districts(name)";
...
export async function updateEvent(eventId, f) {
  ...
  const { error } = await supabase.from("events").update({
    ...
    skill_level: LEVEL_TO_DB[f.level] || "farketmez",
  }).eq("id", eventId);
  if (error) throw error;
