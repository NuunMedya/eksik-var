  ...
  const UserRow = ({ u }) => (
    <TouchableOpacity onPress={() => openUser(u)} style={st.row}>
      ...
      <View style={{ alignItems: "flex-end", gap: 2 }}>
        ...
        <Text style={{ fontSize: 11, fontWeight: "700", color: (u.rel != null && u.rel < 85) ? C.kit : C.pitch }}>%{u.rel != null ? u.rel : 100}</Text>
      </View>
    </TouchableOpacity>
  );
    ...
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      {showEmpty ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={loading ? <ActivityIndicator color={C.pitch} style={{ marginBottom: 10 }} /> : null}
