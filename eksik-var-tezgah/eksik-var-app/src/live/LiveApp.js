...
import { cloneForRepeat, CATEGORIES, POSITIONS, GUNLER } from "../data";
                ...
                <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 30 }} keyboardShouldPersistTaps="handled">
                  <View style={{ backgroundColor: C.surface, borderRadius: 20, padding: 16, shadowColor: "#000", shadowOpacity: 0.1, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 3 }}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 12 }} contentContainerStyle={{ gap: 7 }}>
                      {CATEGORIES.map((k) => (
                        <TouchableOpacity key={k.id} onPress={() => { setVitrinCat(k.id); const g2 = new Set((POSITIONS[k.id] || []).map((p2) => p2.id)); setVitrinPos((eski) => eski.filter((x) => g2.has(x))); }}
                    ...
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 12 }}>
                      ...
                      {me.catLevels && me.catLevels[vitrinCat] && (
                        <View style={{ backgroundColor: C.kitSoft, borderRadius: 999, paddingHorizontal: 11, paddingVertical: 6 }}>
                          <Text style={{ fontSize: 12.5, fontWeight: "800", color: C.kit }}>{me.catLevels[vitrinCat]}</Text>
                        </View>
                      )}
                    </View>
