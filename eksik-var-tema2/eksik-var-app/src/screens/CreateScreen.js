    ...
    >
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
        ...
        {rakip ? (
          <>
            <Field label={t("Takımınızın adı")}>
              <Input placeholder={t("Örn: Keçiören Kartalları")} value={f.teamName} onChangeText={set("teamName")} />
            </Field>
          ...
          <Field label={t("Etkinlik başlığı")}>
            <Input placeholder={t("Örn: Perşembe Halı Saha")} value={f.title} onChangeText={set("title")} />
          </Field>
        )}
        ...
        <Field label={t("Saha / mekan")}>
          ...
          <Input placeholder={t("Örn: Yıldız Halı Saha, Çankaya")} value={f.venue} onChangeText={set("venue")} />
        </Field>
        )}
