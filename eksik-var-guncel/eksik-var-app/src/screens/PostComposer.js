...
import { pickPhoto, pickVideo } from "../avatar";
...
export default function PostComposer({ visible, onClose, hasListing = false, onShare, busy = false }) {
  ...
  useEffect(() => { if (visible) { setCaption(""); setImageUri(null); setVideoUri(null); setAttach(false); } }, [visible]);

  const gecerli = caption.trim().length > 0 || !!imageUri || !!videoUri;
  const paylas = () => { if (gecerli && !busy) onShare({ caption: caption.trim(), imageUri, videoUri, attach: attach && hasListing }); };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={st.backdrop}>
        ...
        <View style={st.sheet}>
          ...
          {imageUri ? (
            ...
            <View style={st.videoCip}>
              <Text style={{ fontSize: 16 }}>🎬</Text>
              <Text style={{ flex: 1, fontSize: 13, fontWeight: "800", color: C.turfText }}>{t("Video hazır — paylaşınca yüklenecek")}</Text>
              <TouchableOpacity onPress={() => setVideoUri(null)} style={{ padding: 4 }}><Ionicons name="close" size={16} color={C.faint} /></TouchableOpacity>
            </View>
          ) : (
            <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
              <TouchableOpacity onPress={async () => { const u = await pickPhoto(); if (u) setImageUri(u); }} style={[st.fotoBtn, { flex: 1, marginTop: 0 }]}>
                <Ionicons name="image-outline" size={18} color={C.turfText} />
                <Text style={{ fontSize: 13, fontWeight: "800", color: C.turfText }}>{t("Fotoğraf")}</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={async () => { const u = await pickVideo(); if (u) setVideoUri(u); }} style={[st.fotoBtn, { flex: 1, marginTop: 0, borderColor: C.kit }]}>
                <Ionicons name="videocam-outline" size={18} color={C.kit} />
                <Text style={{ fontSize: 13, fontWeight: "800", color: C.kit }}>{t("Video (30 sn)")}</Text>
              </TouchableOpacity>
            </View>
          )}
...
const mkSt = () => StyleSheet.create({
  ...
  videoCip: { flexDirection: "row", alignItems: "center", gap: 8, marginTop: 8, backgroundColor: C.kitSoft, borderRadius: 12, borderWidth: 1.5, borderStyle: "dashed", borderColor: C.kit, paddingHorizontal: 10, paddingVertical: 10 },
