...
import { C } from "../theme";
import { CATEGORIES, GUNLER_UZUN, positionSlots, posLabel, posIcon, venueModeLabel, costModeLabel, teamLabel, VARMISIN_OPTIONS, PAYMENT_LABEL, paymentSummary, formatLabel, relInfo, relColor, eventPhase } from "../data";
    ...
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 140 }}>
          ...
          <View style={st.card}>
            ...
            {ev.mine && phase !== "tamamlandi" && (
              <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
                <TextInput value={guestName} onChangeText={setGuestName} placeholder="Ad Soyad" placeholderTextColor="#9AA79F" style={[st.scoreInput, { flex: 1, width: undefined, fontSize: 14, fontWeight: "600", textAlign: "left", paddingHorizontal: 12 }]} />
        ...
        {ev.joined && !ev.mine && (phase === "bugun" || phase === "oynandi") && ev.status !== "tamamlandi" && (
          <TouchableOpacity disabled={!!ev.checkedIn} onPress={() => onCheckIn(ev.id)} style={[st.cta, { backgroundColor: ev.checkedIn ? C.pitchSoft : C.pitch, marginTop: 10, flexDirection: "row", justifyContent: "center", gap: 8 }]}>
            <Ionicons name={ev.checkedIn ? "checkmark-circle" : "location"} size={18} color={ev.checkedIn ? C.turf : "#fff"} />
            <Text style={[st.ctaText, ev.checkedIn && { color: C.turf }]}>{ev.checkedIn ? "Sahadayım dedin ✓ · organizatör yoklamada görür" : "Sahadayım"}</Text>
          </TouchableOpacity>
        )}
        {upcoming && (
          <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
            {ev.venue ? (
              <TouchableOpacity onPress={() => onDirections(ev)} style={[st.btn, st.btnGhost, { flex: 1, flexDirection: "row", gap: 6, paddingVertical: 10 }]}>
                <Ionicons name="navigate-outline" size={16} color={C.turf} /><Text style={[st.btnText, { color: C.turf }]}>Yol tarifi</Text>
              </TouchableOpacity>
            ) : null}
            {(ev.joined || ev.mine) && (
              <TouchableOpacity onPress={() => onCalendar(ev)} style={[st.btn, st.btnGhost, { flex: 1, flexDirection: "row", gap: 6, paddingVertical: 10 }]}>
                <Ionicons name="calendar-outline" size={16} color={C.turf} /><Text style={[st.btnText, { color: C.turf }]}>Takvime ekle</Text>
              </TouchableOpacity>
            )}
          </View>
        )}
