...
export default function DemoApp() {
  ...
  const saveAttendance = (eventId, marks) => {
    ...
    const payments = ev.price > 0 ? rosterFor(ev).map((m) => ({ id: m.id, name: m.name, avatar: m.avatar, amount: ev.price, status: marks[m.id] === "gelmedi" ? "muaf" : "bekliyor" })) : [];
    setEvents((es) => es.map((e) => (e.id === eventId ? { ...e, status: "tamamlandi", attendance: marks, payments } : e)));
          ...
          {activeEvent && (
            <View style={StyleSheet.absoluteFill}>
              <EventDetailScreen
                ...
                availability={availabilityFor(activeEvent, chats)}
