...
import { Field, Input, Chip, PickerRow, PickerSheet, BACK_ICON } from "../components";
...
export default function CreateScreen({ user, onBack, onCreate, initial = null, preset = null, repeat = false, onListVenues = null, onAddVenue = null }) {
  ...
  const [f, setF] = useState(() => initial ? {
    ...
    title: "", cat: (user.favCats && user.favCats[0]) || 1, city: user.city || "Ankara", district: user.district || "", venue: "",
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
        ...
        {rakip ? (
          <>
            ...
            <Field label={t("Saha")}>
              <View style={{ flexDirection: "row", flexWrap: "wrap", rowGap: 8 }}>
                {VENUE_MODES.map(([id, label]) => <Chip key={id} label={t(label)} active={f.venueMode === id} onPress={() => setF((p) => ({ ...p, venueMode: id, ...(id !== "bizde" ? { venue: "", venueId: null, venueLat: null, venueLng: null } : {}) }))} />)}
              </View>
              {f.venueMode === "bizde" && (
