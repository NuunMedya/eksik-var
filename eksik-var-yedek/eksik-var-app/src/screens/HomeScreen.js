...
import { C } from "../theme";
import { CATEGORIES, GUNLER_UZUN, districtCounts, sortDistricts, needsSummary, matchesMyPositions } from "../data";
...
export default function HomeScreen({ user, events, onOpen, onAttendance, onChangeCity, onNotifications, unreadCount = 0, blockedIds = [] }) {
  ...
  const list = events.filter(
    (e) => !e.ended && e.status !== "iptal" && e.city === user.city && !(e.org && blockedIds.includes(e.org.id)) &&
