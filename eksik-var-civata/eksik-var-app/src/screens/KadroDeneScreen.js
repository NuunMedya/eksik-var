...
function Tas({ x, y, bg, fg, ad, onDrag = () => {} }) {
  const pan = useRef(new Animated.ValueXY()).current;
  const pr = useRef(PanResponder.create({
    onStartShouldSetPanResponder: () => true,
    onMoveShouldSetPanResponder: () => true,
    onPanResponderTerminationRequest: () => false,
    onPanResponderGrant: () => onDrag(true),
    onPanResponderMove: Animated.event([null, { dx: pan.x, dy: pan.y }], { useNativeDriver: false }),
    onPanResponderRelease: () => { pan.extractOffset(); onDrag(false); },
...
function Yarim({ ust, form, adlar, renk, onDrag }) {
  const [bg, fg] = RENKLER[renk];
  const parcalar = parcala(form);
  const satirlar = [[1], ...parcalar.map((n2) => Array.from({ length: n2 }))].map((r, ri) => ({ ri, n: r.length }));
    ...
    <View style={{ position: "absolute", left: 0, right: 0, top: ust ? 0 : "50%", height: "50%" }}>
      {satirlar.map(({ ri, n }) => {
        ...
        return Array.from({ length: n }).map((_, j) => {
          const slot = ri === 0 ? 0 : 1 + parcalar.slice(0, ri - 1).reduce((a2, b2) => a2 + b2, 0) + j;
          return <Tas key={form + renk + ri + "-" + j} x={((j + 1) / (n + 1)) * 100} y={y} bg={bg} fg={fg} ad={adlar[slot] || ""} onDrag={onDrag} />;
        });
      })}
    </View>
  );
...
export default function KadroDeneScreen({ onBack, onSearch = null, meName = null }) {
  ...
  const [tasSurukleniyor, setTasSurukleniyor] = useState(false);
  ...
  const sonuctan = (u) => {
    const hedef = aramaHedef;
    setAramaHedef(null); setQ(""); setSonuc([]);
    if (!hedef) return;
    if ([...boyut(listA), ...boyut(listB)].some((x) => x.trim() === u.name)) return;
    const set = hedef === "A" ? setListA : setListB;
    set((l) => { const y = boyut(l); if (y.some((x) => x.trim() === u.name)) return y; const bos = y.findIndex((x) => !x.trim()); if (bos >= 0) y[bos] = u.name; return y; });
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      <ScrollView scrollEnabled={!tasSurukleniyor} contentContainerStyle={{ padding: 14, paddingBottom: 30 }}>
        ...
        <View ref={sahaRef} collapsable={false} style={{ borderRadius: 20, overflow: "hidden", backgroundColor: "#1B5E3B" }}>
          ...
          <View style={{ height: 560 }}>
            ...
            <Yarim ust form={formA} adlar={boyut(listA).map(varsayilan)} renk={renkA} onDrag={setTasSurukleniyor} />
            <Yarim ust={false} form={formB} adlar={boyut(listB).map(varsayilan)} renk={renkB} onDrag={setTasSurukleniyor} />
