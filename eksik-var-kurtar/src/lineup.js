...
export function makeTeams(players, mode = "dengeli") {
  ...
  // mevki grupları (defans / orta / forvet / diğer) ayrı ayrı dengelenir: her takıma benzer sayıda defans ve forvet
  const groupOf = (p) => { const ps = p.positions || []; return ps.includes("defans") ? "defans" : ps.includes("orta") ? "orta" : ps.includes("forvet") ? "forvet" : "diger"; };
  ["defans", "orta", "forvet", "diger"].forEach((grp) => {
    rest.filter((p) => groupOf(p) === grp).sort((a, b) => (b.rating || 0) - (a.rating || 0)).forEach((p) => {
