...
async function sorgula(il, spor) {
  ...
  for (let deneme = 0; deneme < 6; deneme++) {
    ...
    try {
      ...
      let r = await fetch(ayna, { method: "POST", body: "data=" + encodeURIComponent(q), headers: basliklar });
      if (!r.ok && r.status !== 429 && r.status !== 504) {
        r = await fetch(ayna + "?data=" + encodeURIComponent(q), { headers: { "User-Agent": basliklar["User-Agent"], "Accept": "application/json" } });
      }
      ...
      process.stdout.write(` (tekrar: ${e.message})`);
