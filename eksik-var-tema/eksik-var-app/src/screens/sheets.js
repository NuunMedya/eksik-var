    ...
    <SheetShell visible={visible} onClose={onClose}>
      ...
      {!yedek && !rakip && (
        <View style={{ marginTop: 10 }}>
          <Text style={st.label}>HAZIR CÜMLE</Text>
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
            {[chosen === "kaleci" ? "Kaleye geçerim, eldiven bende 🧤" : "Ben varım, saatinde oradayım 👍", "İlk kez geliyorum, tempoya ayak uydururum", "Arabam var, yol üstünden alabilirim"].map((t) => (
              <TouchableOpacity key={t} onPress={() => setNote(t)} style={st.posChip}><Text style={{ fontSize: 12, fontWeight: "700", color: C.ink }}>{t}</Text></TouchableOpacity>
            ))}
          </View>
        </View>
      )}
...
export function PollSheet({ visible, onClose, onSend }) {
  ...
  return (
    <SheetShell visible={visible} onClose={() => { reset(); onClose(); }}>
      ...
      <TextInput value={q} onChangeText={setQ} placeholder="Örn: Bu hafta hangi gün?" placeholderTextColor={C.placeholder} style={st.input} />
      <Text style={[st.label, { marginTop: 12 }]}>SEÇENEKLER</Text>
      {opts.map((o, i) => (
        <View key={i} style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 }}>
          <TextInput value={o} onChangeText={(v) => setOpt(i, v)} placeholder={`Seçenek ${i + 1}`} placeholderTextColor={C.placeholder} style={[st.input, { flex: 1, marginTop: 0 }]} />
          {opts.length > 2 && (
