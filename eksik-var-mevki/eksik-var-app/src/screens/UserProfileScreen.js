...
import { posLabel, posIcon } from "../data";

export default function UserProfileScreen({ user, comments = [], rules, blocked = false, loading = false,
  onBack, onMessage, onCall, onBlock, onReport }) {
  if (!user) return null;
  const rel = user.rel == null ? 100 : user.rel;
  const confirmBlock = () =>
    blocked ? onBlock(user)
      : Alert.alert("Engelle", `${user.name} artık sana yazamaz, arayamaz; etkinliklerini görmezsin.`,
          [{ text: "Vazgeç", style: "cancel" }, { text: "Engelle", style: "destructive", onPress: () => onBlock(user) }]);
    ...
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      <View style={st.header}>
        ...
        <Text style={{ color: C.mist, fontSize: 13 }}>
          @{user.username}{user.city ? ` · ${user.district ? user.district + ", " : ""}${user.city}` : ""}
        </Text>
        <View style={{ flexDirection: "row", gap: 6, marginTop: 8 }}>
          {user.verified && (
            <View style={st.tag}><Ionicons name="shield-checkmark" size={12} color={C.star} /><Text style={st.tagText}>Doğrulanmış</Text></View>
          )}
          {user.role === "organizator" && (<View style={st.tag}><Text style={st.tagText}>Organizatör</Text></View>)}
          {blocked && (<View style={[st.tag, { backgroundColor: C.kit }]}><Text style={st.tagText}>Engelledin</Text></View>)}
        </View>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }}>
        <View style={st.statsRow}>
          ...
          <View style={st.statBox}>
            <Ionicons name="shield-checkmark" size={16} color={rel < 85 ? C.kit : C.pitch} />
            <Text style={[st.statBig, rel < 85 && { color: C.kit }]}>%{rel}</Text>
            <Text style={st.statSub}>güvenilirlik</Text>
          </View>
          <View style={st.statBox}>
            <Ionicons name="football-outline" size={16} color={C.turf} />
        ...
        <View style={st.card}>
          ...
          {!loading && comments.length === 0 && (
            <Text style={{ color: C.faint, fontSize: 13, marginTop: 8 }}>Henüz yorum yok. Birlikte oynadıktan sonra ilk yorumu sen yazabilirsin.</Text>
          )}
          {comments.map((c, i) => (
        ...
        {!user.isMe && onBlock && (
          <View style={{ flexDirection: "row", justifyContent: "center", gap: 22, marginTop: 16 }}>
            ...
            <TouchableOpacity onPress={() => onReport(user)} style={st.minor}>
              ...
              <Text style={st.minorText}>Şikayet et</Text>
            </TouchableOpacity>
          </View>
        )}
      </ScrollView>
...
const st = StyleSheet.create({
  ...
  statBox: { flex: 1, backgroundColor: "#fff", borderRadius: 16, paddingVertical: 12, alignItems: "center", gap: 3, elevation: 2, shadowColor: "#000", shadowOpacity: 0.06, shadowRadius: 4, shadowOffset: { width: 0, height: 2 } },
  statBig: { fontSize: 17, fontWeight: "900", color: C.turf },
  statSub: { fontSize: 10, color: C.faint, textAlign: "center" },
