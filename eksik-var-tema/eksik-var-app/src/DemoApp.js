...
export default function DemoApp() {
  ...
  const claimPayment = (eventId) => {
    ...
    showToast("Organizatöre bildirildi · onaylayınca 'Ödendi' olur");
  ...
  const checkInWithCode = (eventId, code) => { if (code.length !== 4) { showToast("4 haneli kod"); return; } checkIn(eventId); };
  ...
  const confirmAllPayments = (eventId) => {
    Alert.alert("Herkes ödedi", "Bekleyen ve 'ödedim' diyen herkes 'ödendi' olarak işaretlenecek.", [{ text: "Vazgeç", style: "cancel" }, { text: "İşaretle", onPress: () => {
      setEvents((es) => es.map((e) => (e.id === eventId ? { ...e, payments: (e.payments || []).map((p) => (p.status === "bekliyor" || p.status === "odedim" ? { ...p, status: "odendi" } : p)) } : e)));
      showToast("Tüm ödemeler işaretlendi ✓");
    } }]);
  };
  ...
  const sendIban = (eventId) => {
    ...
    if (!paymentDetails) { showToast("Önce Ayarlar > Ödeme bilgileri'ne IBAN'ını yaz"); setView({ name: "settings" }); return; }
    ...
    showToast("IBAN gruba gönderildi 💳");
  };
  ...
  const saveIban = (d) => { setPaymentDetails(d); showToast("Ödeme bilgilerin kaydedildi 💳"); };
  ...
  const recordScore = (eventId, home, away) => {
    ...
    showToast(`Skor kaydedildi · ${home} – ${away}`);
  };
  ...
  const askAvailability = (eventId) => {
    ...
    showToast("Sabit kadroya soruldu · cevaplar geldikçe özet güncellenir");
  ...
  const applySuggested = (eventId) => {
    ...
    showToast(`Eksik ${av.suggested} olarak güncellendi · kadroya duyuruldu`);
  };

  /* --- misafir oyuncular --- */
  const addGuest = (eventId, name) => {
    const g = { id: "g-" + uid(), name, available: true, attendance: "bekleniyor", payment: "bekliyor" };
    setEvents((es) => es.map((e) => (e.id === eventId ? { ...e, guests: [...(e.guests || []), g] } : e)));
    showToast(`${name} eklendi · yoklama, ödeme ve kurada yer alır`);
  };
  ...
  const checkIn = (eventId) => { setEvents((es) => es.map((e) => (e.id === eventId ? { ...e, checkedIn: true } : e))); showToast("Sahadayım dedin ✓ organizatör yoklamada görecek"); };
      ...
      <SafeAreaView edges={["left", "right", "bottom"]} style={{ flex: 1, backgroundColor: C.surface }}>
        <View style={{ flex: 1, backgroundColor: C.chalk }}>
          ...
          {view.name === "editProfile" && (
            <View style={StyleSheet.absoluteFill}>
              <EditProfileScreen user={user} onBack={() => setView({ name: "root" })}
                onSave={(f) => { setUser((u) => ({ ...u, ...f, district: f.district || null })); setView({ name: "root" }); showToast("Profilin güncellendi"); }} />
            </View>
          )}
          {view.name === "blocked" && (
            <View style={StyleSheet.absoluteFill}>
