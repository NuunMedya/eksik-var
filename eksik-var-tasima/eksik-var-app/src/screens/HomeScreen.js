...
export default function HomeScreen({ user, events, onOpen, onAttendance, onChangeCity, onNotifications, unreadCount = 0, blockedIds = [], onCreate = () => {}, onBringTeam = () => {}, loading = false, offline = false, sponsors = [], onSponsor = () => {}, market = [], onOpenPlayer = () => {}, onEditListing = () => {}, onOfferPlayer = null, initialKind = "oyuncu" , onCreateRakip, onMatches = () => {} }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      ...
      {kind !== "oyuncu" && (
        <TouchableOpacity onPress={() => setKind("oyuncu")} style={{ paddingHorizontal: 16, paddingTop: 10 }}>
          <Text style={{ fontSize: 13, fontWeight: "800", color: C.faint }}>‹ {t("İlanlara dön")}</Text>
        </TouchableOpacity>
      )}
