...
import { cloneForRepeat, CATEGORIES, posIcon, posLabel } from "../data";
...
export default function LiveApp() {
  ...
  const saveListing = async (f) => { try { await api.upsertListing(meId, user, f); await refreshMarket(); setVitrinYazi((f.bio || "").trim()); setVitrinUcret(""); setVitrinOniz(f); } catch (e) { fail(e); } };
                ...
                <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 30 }} keyboardShouldPersistTaps="handled">
                  <View style={{ backgroundColor: C.surface, borderRadius: 20, padding: 16, shadowColor: "#000", shadowOpacity: 0.1, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 3 }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                      ...
                      <Text style={{ fontSize: 30 }}>{(CATEGORIES.find((k) => k.id === vitrinOniz.cat) || {}).icon || "\u26BD"}</Text>
                    </View>
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 14 }}>
                      {((vitrinOniz.positions && vitrinOniz.positions.length ? vitrinOniz.positions : me.positions) || []).map((p2) => (
                        <View key={p2} style={{ backgroundColor: C.pitchSoft, borderRadius: 999, paddingHorizontal: 11, paddingVertical: 6 }}>
                          <Text style={{ fontSize: 12.5, fontWeight: "800", color: C.turfText }}>{posIcon(p2)} {posLabel(p2)}</Text>
                        </View>
                      ))}
                      {me.catLevels && me.catLevels[vitrinOniz.cat] && (
                        <View style={{ backgroundColor: C.kitSoft, borderRadius: 999, paddingHorizontal: 11, paddingVertical: 6 }}>
                          <Text style={{ fontSize: 12.5, fontWeight: "800", color: C.kit }}>{me.catLevels[vitrinOniz.cat]}</Text>
                        </View>
                      )}
                    </View>
                    {(vitrinOniz.days || []).length > 0 && (
                      <Text style={{ fontSize: 12.5, color: C.faint, marginTop: 10 }}>{"\u{1F4C5} "}{t("Müsait")}: {(vitrinOniz.days || []).join(" · ")}</Text>
                    )}
