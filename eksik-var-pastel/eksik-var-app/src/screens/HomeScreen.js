...
export default function HomeScreen({ user, events, onOpen, onAttendance, onChangeCity, onNotifications, unreadCount = 0, blockedIds = [], onCreate = () => {}, onBringTeam = () => {}, loading = false, offline = false, sponsors = [], onSponsor = () => {}, market = [], onOpenPlayer = () => {}, onEditListing = () => {}, onOfferPlayer = null, initialKind = "oyuncu" , onCreateRakip, onMatches = () => {}, onOpenShowcase = () => {} }) {
  ...
  const canli = events.filter((e) => !e.ended && e.status !== "iptal" && e.city === user.city && !(e.org && blockedIds.includes(e.org.id)));
  const oyuncuSay = canli.filter((e) => (e.kind || "oyuncu") === "oyuncu").length;
  const rakipSay = canli.filter((e) => e.kind === "rakip").length;
    ...
    <View style={{ flex: 1, backgroundColor: C.chalk }}>
      <View style={st.header}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <View style={{ backgroundColor: "rgba(255,255,255,0.14)", borderRadius: 999, paddingHorizontal: 5, paddingVertical: 3, flexDirection: "row", gap: 2 }}>
            {[1, ...(user.favCats || []).filter((c) => c !== 1)].slice(0, 4).map((cid) => (
              <TouchableOpacity key={cid} onPress={() => setCat(cat === cid ? 0 : cid)}
                style={{ width: 26, height: 26, borderRadius: 13, alignItems: "center", justifyContent: "center", backgroundColor: cat === cid ? "rgba(255,255,255,0.4)" : "transparent" }}>
                <Text style={{ fontSize: 13.5 }}>{(CATEGORIES.find((c) => c.id === cid) || {}).icon}</Text>
              </TouchableOpacity>
            ))}
          </View>
          <TouchableOpacity onPress={onMatches} style={{ padding: 4 }}>
            <Ionicons name="calendar-outline" size={21} color={C.mist} />
          </TouchableOpacity>
      ...
      <View style={{ flexDirection: "row", marginHorizontal: 16, marginTop: 10, backgroundColor: C.surface, borderRadius: 14, borderWidth: 1, borderColor: C.line, padding: 4, gap: 4 }}>
        {[["oyuncu", "\u{1F9E4} " + t("Oyuncu arayanlar"), oyuncuSay], ["rakip", "\u{1F19A} " + t("Rakip arayanlar"), rakipSay]].map(([k2, ad, say]) => (
          <TouchableOpacity key={k2} onPress={() => setKind(k2)}
            style={{ flex: 1, borderRadius: 10, paddingVertical: 9, alignItems: "center", backgroundColor: kind === k2 ? C.turf : "transparent" }}>
            <Text style={{ fontSize: 12.5, fontWeight: "900", color: kind === k2 ? "#fff" : C.faint }}>{ad}{say > 0 ? " · " + say : ""}</Text>
          </TouchableOpacity>
        ))}
      </View>
          ...
          <View>
            ...
            {kind === "oyuncu" && (
              <TouchableOpacity onPress={onOpenShowcase} activeOpacity={0.85}
                style={{ flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: C.pitchSoft, borderWidth: 1.5, borderColor: C.turf, borderRadius: 16, padding: 13, marginBottom: 12 }}>
                <Text style={{ fontSize: 22 }}>🎽</Text>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 14, fontWeight: "900", color: C.turfText }}>{t("Oyuncuyum, takım arıyorum")}</Text>
                  <Text style={{ fontSize: 11, color: C.faint }}>{t("Oyuncu ilanını paylaş, teklifler gelsin")}</Text>
                </View>
        ...
        ListEmptyComponent={loading ? null :
          ...
          <View style={st.empty}>
            ...
            <Text style={{ color: C.faint, fontSize: 13, marginTop: 4, textAlign: "center" }}>
              {kind === "pazar" ? t("İlk vitrini sen aç; puanın ve güvenilirliğin kartında hazır.") : kind === "rakip" ? "İlk rakip ilanını sen aç; ilindeki takım kaptanlarına bildirim gider." : "Yakınında ilan açıldığında sana bildirim gelir."}
            </Text>
...
const mkSt = () => StyleSheet.create({
  ...
  kindRow: { flexDirection: "row", gap: 6, paddingHorizontal: 16, paddingTop: 10 },
