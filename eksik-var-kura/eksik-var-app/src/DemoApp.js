...
export default function DemoApp() {
  ...
  const sohbetAyar = (c, patch, mesaj) => { setChats((l) => l.map((x) => x.id === c.id ? { ...x, ...patch } : x)); if (mesaj) showToast(mesaj); };
  const [yorumlar, setYorumlar] = useState(null);
  const [demoYorum, setDemoYorum] = useState({});
  ...
  const openNotif = (n) => {
    if (n.type === "teklif") { setView({ name: "teklifler" }); return; }
  ...
  const hideTabs = ["chat", "create", "settings", "groupInfo", "attendance", "notifications", "rate", "blocked", "profile", "lineup", "editProfile", "penalti", "kadroDene", "basket", "voleybol", "tenis", "teklifler", "kulup", "takim", "arena"].includes(view.name);
  const ustEkran = ["hub", "penalti", "kadroDene", "basket", "voleybol", "tenis", "teklifler", "kulup", "takim", "arena"].includes(view.name) || !!activeEvent || !!call;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.turf }}>
      ...
      <SafeAreaView edges={["left", "right", "bottom"]} style={{ flex: 1, backgroundColor: C.surface }}>
        <View style={{ flex: 1, backgroundColor: C.chalk }}>
          {tab === "home" && !ustEkran && view.name !== "event" && (
            <HomeScreen
              ...
              onCreate={() => setView({ name: "create" })} onCreateRakip={() => setView({ name: "create", preset: "rakip" })} onBringTeam={() => setView({ name: "create", preset: "ekip" })}
            />
          )}
          {tab === "search" && !ustEkran && (
            <SearchScreen onSearchUsers={searchUsers} onSearchEvents={searchEvents} suggestions={suggestions}
              onCommentsPost={(p) => setYorumlar({ post: p, rows: demoYorum[p.id] || [], busy: false })}
              onEditPost={(p) => Alert.prompt && Alert.prompt(t("Düzenle"), t("Açıklamayı güncelle"), [{ text: t("Vazgeç"), style: "cancel" }, { text: t("Kaydet"), onPress: (v) => setPosts((ps) => ps.map((pp) => pp.id === p.id ? { ...pp, caption: (v || "").trim() } : pp)) }], "plain-text", p.caption || "")}
              onArchivePost={(p) => { setPosts((ps) => ps.filter((pp) => pp.id !== p.id)); showToast(t("Arşivlendi 🗄")); }}
          ...
          {tab === "chats" && !ustEkran && view.name !== "chat" && <ChatsScreen chats={chats} onOpen={openChat}
            onNew={() => Alert.alert(t("Yeni sohbet"), "", [
              ...
              { text: t("Vazgeç"), style: "cancel" }])}
            onMute={(c) => sohbetAyar(c, { muted: !c.muted }, c.muted ? t("Ses açıldı 🔔") : t("Sessize alındı 🔕"))}
            onArchive={(c) => sohbetAyar(c, { archived: !c.archived }, c.archived ? t("Arşivden çıktı 📤") : t("Arşivlendi 🗄"))}
            onHide={(c) => sohbetAyar(c, { hiddenAt: new Date().toISOString(), lastRaw: null }, t("Sohbet kaldırıldı 🗑"))} />}
          ...
          {tab === "profile" && !ustEkran && view.name !== "settings" && (
            <ProfileScreen
              ...
              market={market} onOpenPlayer={(p) => p.userId === "me" ? setTab("profile") : openProfile(p.userId)}
              ...
              onEdit={() => setView({ name: "editProfile", bolum: "kimlik" })}
          ...
          {view.name === "takim" && <TakimScreen meId="me"
            onTeamChat={() => { const c = chats.find((x) => x.type === "grup" && x.title.includes(team ? team.name : "")); if (c) { setTab("chats"); openChat(c.id); } else { const yeni = { id: uid(), type: "grup", title: "🛡 " + (team ? team.name : "Takımım"), sub: "takım grubu", msgs: [{ id: uid(), from: "sys", text: "Takım grubu kuruldu 🛡", time: "şimdi" }], unread: 0, lastTime: "şimdi", muted: false, archived: false, teamId: "t1" }; setChats((l) => [yeni, ...l]); setTab("chats"); openChat(yeni.id); } }}
            ...
            onLogoPick={async () => { showToast(t("Logo yükleme canlı modda 📷")); return null; }}
            onAddGuest={() => showToast(t("Misafir ekleme canlı modda"))} onRemoveGuest={() => {}} />}
          ...
          {view.name === "kulup" && <ClubBoardScreen meId="me" myTeam={null}
            rakipRows={events.filter((e) => e.kind === "rakip")} onOpenRakip={(e) => { setTab("home"); setView({ name: "event", id: e.id }); }}
          ...
          {view.name === "kadroDene" && <KadroDeneScreen onSearch={async (q) => searchUsers(q)} meName={user.name} onBack={() => setView({ name: "hub" })} />}
          {view.name === "settings" && (
          ...
          {view.name === "editProfile" && (
            <View style={StyleSheet.absoluteFill}>
              <EditProfileScreen bolum={view.bolum || "hepsi"} onSaveIdentity={(f) => { setUser((u) => ({ ...u, favCats: f.favCats, positions: f.positions, catLevels: f.levels || {} })); showToast(t("Kaydedildi \u2713")); }} user={user} marketMine={myListing} onSaveListing={saveListing} onDropListing={dropListing} onBack={() => setView({ name: "root" })}
        ...
        onList={(q) => Promise.resolve(filterVenues(communityVenues.filter((v) => v.city === user.city && v.category_id === hubCat), q))}
        onAdd={(name, lat, lng) => Promise.resolve({ name, lat, lng })}
        onPick={(v) => { setVenueHub(false); yolSec(v.name, v.lat, v.lng, `${v.name} ${user.city}`); }} />
      {yeniSohbet && (
        <NewChatModal mode={yeniSohbet} busy={false}
          onClose={() => setYeniSohbet(null)}
          onSearch={async (q) => searchUsers(q)}
          onCreateDirect={(u) => { const c = { id: uid(), type: "birebir", title: u.name, sub: "", msgs: [], unread: 0, lastTime: "şimdi", muted: false, archived: false }; setChats((l) => [c, ...l]); setYeniSohbet(null); setTab("chats"); openChat(c.id); }}
          onCreateGroup={(ad, users) => { const c = { id: uid(), type: "grup", title: ad, sub: `${users.length + 1} üye`, msgs: [{ id: uid(), from: "sys", text: "Grup kuruldu 👥", time: "şimdi" }], unread: 0, lastTime: "şimdi", muted: false, archived: false, members: users }; setChats((l) => [c, ...l]); setYeniSohbet(null); setTab("chats"); openChat(c.id); showToast(t("Grup kuruldu 👥")); }} />
      )}
      {yorumlar && (
        <CommentsSheet post={yorumlar.post} rows={yorumlar.rows} meId="me" busy={false}
          onClose={() => setYorumlar(null)}
          onSend={(b2) => { const y = { id: uid(), body: b2, createdAt: new Date().toISOString(), userId: "me", name: user.name, avatar: user.avatar };
            setDemoYorum((m2) => ({ ...m2, [yorumlar.post.id]: [...(m2[yorumlar.post.id] || []), y] }));
            setYorumlar((x) => ({ ...x, rows: [...x.rows, y] }));
            setPosts((ps) => ps.map((pp) => pp.id === yorumlar.post.id ? { ...pp, yorumSayi: (pp.yorumSayi || 0) + 1 } : pp)); }}
          onDelete={(id2) => { setDemoYorum((m2) => ({ ...m2, [yorumlar.post.id]: (m2[yorumlar.post.id] || []).filter((r) => r.id !== id2) }));
            setYorumlar((x) => ({ ...x, rows: x.rows.filter((r) => r.id !== id2) })); }} />
      )}
